...
const storageSet = async (id, value, mode) => {
  return await promisify(chrome.storage[mode].set.bind(chrome.storage[mode]))(Object.fromEntries([
    [id, value]
  ]))
}
const cookieGet = async () => {
  return Object.fromEntries((await promisify(chrome.cookies.getAll)({
