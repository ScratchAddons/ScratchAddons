...
async function getUserscripts() {
  for (const { manifest, addonId } of scratchAddons.manifests) {
    if (manifest.userscripts || manifest.userstyles) {
      addonsWithScriptsOrStyles.push({ addonId, scripts: manifest.userscripts || [], styles: manifest.userstyles || [] });
    }
  }
}
...
async function sendUserscriptsAndUserstyles(url, tabId) {
  ...
  for (const addon of data) {
    for (const i in addon.styleUrls) {
      promises.push(fetch(chrome.runtime.getURL(`/addons/${addon.addonId}/${addon.styleUrls[i]}`)).then(res => res.text()).then(text => addon.styles[i] = text));
    }
  }
  ...
  setTimeout(() => {
    if(!receivedResponse) {
      const interval = setInterval(() => chrome.tabs.sendMessage(tabId, {userscriptsAndUserstyles: data}, res => res && clearInterval(interval)), 250);
