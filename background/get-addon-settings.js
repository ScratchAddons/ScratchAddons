...
const transitionToNewStorageKeys = async (addonSettings) => {
...
async function runAddonTransitions() {
   ...
   * @param {string} id addon id
   */
  const createSettingsContext = (id) => {
  ...
  for (const { manifest, addonId } of scratchAddons.manifests) {
    ...
    if (manifest.settings) {
      ...
      if (addonId === "dark-www") {
        ...
        const updatePresetIfMatching = (version, oldPreset = null, presetOrFn = null) => {
          if ((setting("_version") || 0) < version) {
            ...
            for (const key of Object.keys(oldPreset)) {
              if (!areSettingsEqual(setting(key), oldPreset[key])) return console.log(oldPreset, key);
