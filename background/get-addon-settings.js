const migrate = (addonSettings, addonsEnabled) => {
...
const retrieveSettings = () => {
  chrome.storage.sync.get(["addonSettings", "addonsEnabled"], ({ addonSettings = {}, addonsEnabled = {} }) => {
    const func = () => {
      const madeAnyChanges = migrate(addonSettings, addonsEnabled);
      if (madeAnyChanges) {
      ...
      if (!scratchAddons.localState.ready.addonSettings) scratchAddons.localState.ready.addonSettings = true;
    };
...
chrome.storage.onChanged.addListener((changes, areaName) => {
  ...
  if (changes.addonSettings || changes.addonsEnabled) retrieveSettings();
});
