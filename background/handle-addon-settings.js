  ...
  ({ addonSettings = {}, addonsEnabled = {} }) => {
    const func = () => {
      for (const addonId in scratchAddons.manifests) {
        ...
        if (addonsEnabled[addonId] === undefined)
          addonsEnabled[addonId] = !!manifest.enabled_by_default;
      }
...
window.updateAddonSettings = function updateAddonSettings(
  addonId,
  newSettings
) {
...
window.changeEnabledState = function changeEnabledState(addonId, newState) {
  ...
  if (newState === false) {
    const addonObj = scratchAddons.addons.find(
      (addonObj) => addonObj.self.id === addonId
    );
