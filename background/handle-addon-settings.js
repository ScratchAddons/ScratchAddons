...
window.updateAddonSettings = function updateAddonSettings(addonId, newSettings) {
    ...
    chrome.storage.sync.set({addonSettings: scratchAddons.globalState.addonSettings})
}
window.changeEnabledState = function changeEnabledState(addonId, newState) {
    ...
    chrome.storage.sync.set({addonsEnabled: scratchAddons.localState.addonsEnabled});
    if(newState === false) {
