...
export default async function runAddonBgScripts({ addonId, permissions, scripts }) {
    ...
    const clearTimeoutFunc = timeoutId => {
        addonObj._timeouts.splice(addonObj._timeouts.findIndex(x => x===timeoutId), 1);
        return clearTimeout(timeoutId);
    };
    const clearIntervalFunc = intervalId => {
        addonObj._intervals.splice(addonObj._intervals.findIndex(x => x===intervalId), 1);
    ...
    addonObj._revokeProxy = () => {
        scratchAddons.addons.splice(scratchAddons.addons.findIndex(x => x===addonObjReal), 1);
        addonObjRevocable.revoke();
    }
    const globalObj = Object.create(null);
    for(const scriptPath of scripts) {
        const pseudoUrl = `scratchaddons://bgscripts/${addonId}@${scriptPath.split("/").join("@")}`;
        console.log(`%cDebug addons/${addonId}/${scriptPath}: ${pseudoUrl}`, "color:red; font-weight: bold; font-size: 1.3em;");
        let codeToEvaluate = "";
        codeToEvaluate += await (await fetch(`/addons/${addonId}/${scriptPath}`)).text();
        codeToEvaluate += "\n//# sourceURL=" + pseudoUrl;
        evalScript(codeToEvaluate, addonObj, globalObj, setTimeoutFunc, setIntervalFunc, clearTimeoutFunc, clearIntervalFunc);
    }
};