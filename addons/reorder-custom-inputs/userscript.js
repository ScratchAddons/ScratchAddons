export default async function ({ addon, console }) {
    ...
    function createArrow(direction, callback) {
        ...
        const arrowDiv = Blockly.WidgetDiv.DIV.appendChild(document.createElement('div'));
        arrowDiv.innerHTML = `
        ...
        arrowDiv.addEventListener('click', callback);
    }
    ...
    Blockly.ScratchBlocks.ProcedureUtils.shiftFieldCallback = function (field, direction) {
        ...
        if (inputNameToShift && newPosition >= 0 && newPosition < proc.inputList.length) {
            const itemToMove = proc.inputList.splice(proc.inputList.findIndex((input) => input.name === inputNameToShift), 1)[0];
    ...
    ["procedures_declaration", "argument_editor_boolean", "argument_editor_string_number"].forEach((blockType) => {
        if (Blockly.Blocks[blockType]) Blockly.Blocks[blockType].shiftFieldCallback = Blockly.ScratchBlocks.ProcedureUtils.shiftFieldCallback;
    });
