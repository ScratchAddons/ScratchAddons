...
class TokenTypeGroup {
    ...
    *_parseTokens(querier, idx) {
        ...
        for (const parent of this.parents)
            yield* parent._parseTokens(querier, idx);
    }
}
                ...
                for (const field of input.fieldRow) {
                    ...
                    if (field.className_ === "blocklyText blocklyDropdownText") {
                        ...
                        for (let i = 0; i < fieldOptions.length; i++) {
                            if (TokenTypeBlock.INVALID_FIELDS.indexOf(fieldOptions[i][1]) !== -1)
                        ...
                        this.tokenTypeGroups.push(new TokenTypeGroup(null, false, new TokenTypeStringEnum([[field.getText(), null]])));
                    }
                }
    ...
    *parse(querier, idx) {
        for (const value of this._parseSubtoken(idx, 0, querier)) {
            ...
            for (const subtoken of value) {
                ...
                if (subtoken.precedence < this.block.precedence)
                    score += 5;
            }
        ...
        for (const token of tokenGroup.parseTokens(querier, idx)) {
            if (this.block.precedence !== -1) {
                if (token.precedence > this.block.precedence)
                    continue;
                if (token.precedence === this.block.precedence && token.type === this && tokenGroupIdx !== 0)
                    continue;
            }
            ...
            if (input.connection) {
                ...
                if (innerToken.type) {
                    if (innerToken.type instanceof TokenTypeBlock) {
                        ...
                        const innerBlock = input.connection.targetBlock();    
                        if (innerBlock)
                            innerBlock.inputList[0].fieldRow[0].setValue(innerToken.value.toString());
                    }
                }
            }
        }
