export default async function ({ addon, global, console }) {
  ...
  while (1){
    await wait(100)
    try{
      if((await(await fetch("https://clouddata.scratch.mit.edu/logs?projectid="+projectId+"&limit=10&offset=0")).json()).find(e=>e.value==scratchAddons.globalState.temporary[location.hash.slice(1)].publicCode&&e.name.endsWith("oauthgobrrr")))break
    }catch(e){break}
  }
