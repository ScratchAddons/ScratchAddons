...
//// FUNCTIONS 
    ...
    if(oldNum<=1 && oldComments.childNodes.length>1) {console.log('NOT LOADED YET');return}
    oldNum-=2;
    oldNum=Math.min(30,oldNum);
    console.log('LIMIT',oldNum)
    let newComments = await (await fetch(`https://scratch.mit.edu/site-api/comments/user/${username}/?page=1&limit=${oldNum}&rand=${Math.random()}`,{
      headers:{
    ...
    for(let tlr of newDiv.children){
      if(!tlr.classList.contains('top-level-reply')) {continue}
      // console.log(tlr)
      ...
      if(alreadyOnPage) {
        ...
        for(let rli of replies) {
          ...
          if(!ralreadyOnPage) {
            ...
            if(rolderSiblingAlreadyOnPage) { rolderSiblingAlreadyOnPage.after(rli); rolderSiblingAlreadyOnPage.classList.remove('last'); }
            else { alreadyOnPage.parentElement.querySelector('.replies').appendChild(rli) }
          }
        ...
        if(olderSiblingAlreadyOnPage) { olderSiblingAlreadyOnPage.after(tlr); console.log('AFTER!') }
        else if(!olderSiblingAlreadyOnPage && !olderSiblingId) { oldComments.firstElementChild.before(tlr); console.log('AT TOP!') }
        else if(!olderSiblingAlreadyOnPage && olderSiblingId) { oldComments.lastElementChild.after(tlr); console.log('AT BOTTOM!') }
      }
