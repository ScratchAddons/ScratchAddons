...
const msgs = [
  'comments.lengthWarning',
  'comments.muted.duration',
...
// API URLs
const COMMENTS_API_URL = `https://api.scratch.mit.edu${pageType === 'projects' ? `/users/${reduxState.preview.projectInfo.author.username}` : ''}/${pageType}/${pageId}/comments`;
...
const canReport = (author) =>
  pageType !== 'studios' || author !== username;

// regexes to look for URLs & mentions in comments
const urlRegex = /(\b(https?):\/\/[-A-Z0-9+&@#/%?=~_|!:,.;]*scratch\.mit\.edu[-A-Z0-9+&@#/%=~_|]*)/ig;
const mentionRegex = /(?<=(^|[\s.,!])(?!\S*[:#]))[@|＠][a-z0-9_-]{1,20}(?=(?:\b(?!@|＠)|$))/ig;
...
function openModal (type, id) {
      ...
      <div>
        ...
        <div class="flex-row action-buttons">
          <div class="action-buttons-overflow-fix">
            ...
            type === 'report' ? ''
            : `<button class="button action-button submit-button" type="button"><span>${scratchMessage('comments.delete')}</span></button>`
            }
          </div>
        </div>
      </div>
  ...
  modal.querySelector('.modal-content-close')
  .addEventListener('click', closeModal);

  modal.querySelector('.action-buttons-overflow-fix button')
  .addEventListener('click', closeModal);

  modal.querySelector('.action-buttons-overflow-fix :nth-child(2)')
    .addEventListener('click', () => {
      reportComment(id);
      document.getElementById('delete-report-modal')
        .querySelector('.instructions')
        .textContent = scratchMessage('comments.reportModal.reported');
    });

  if (type !== 'report') {
    modal.querySelector('.action-buttons-overflow-fix :nth-child(3)')
      .addEventListener('click', () => {
        deleteComment(id);
      });
  }
...
function deleteComment (id) {
  ...
  document.getElementById(`comments-${id}`).querySelector('.action-list').innerHTML = `<span class="comment-visibility">${scratchMessage('comments.status.deleted')}</span>`;
  closeModal();
}
...
function reportComment (id) {
  ...
  document.getElementById(`comments-${id}`).querySelector('.action-list').innerHTML = `<span class="comment-visibility">${scratchMessage('comments.status.reported')}</span>`;
}
...
function openReplyBox (comment) {
  ...
  replyBox.querySelector('.button.compose-post')
    .addEventListener('click', (evt) => {
      evt.preventDefault();

      postReply(
        comment, evt.target.parentNode.parentNode.parentNode.parentNode.parentNode
      );
    });

  replyBox.querySelector('.button.compose-cancel')
    .addEventListener('click', () => {
...
function closeReplyBox (comment) {
  ...
  commentElement.querySelector('.flex-row.comment-body.column').removeChild(commentElement.querySelector('.flex-row.comment-reply-row'));
}
...
function getMuteMessageInfo () {
  const messageInfo = {
    ...
    spam: {
      muteStepContent: [
        'comment.spam.content1', 'comment.spam.content2'
      ]
    },
    general: {
      ...
      <div class="flex-row">
        ...
        <p class="bottom-text">
          <span>${l10n.get('comments.muted.moreInfoModal', {
            clickHereLink: `<a href="/community_guidelines"><span>${scratchMessage('comments.muted.clickHereLinkText')}</span></a>.`
          })}</span>
        </p>
      </div>
    </div>
...
function postReply (parent, replyBox) {
  ...
  if (replyBox.querySelector('.flex-row.compose-error-row')) {
    replyBox.querySelector('.flex-row.compose-comment.column')
  ...
  const [ commenteeId ] = /\d+/g.exec(document.getElementById(`comments-${parent}`).querySelector('.avatar').src.slice(13));
  ...
  fetch(PROXY_API_URL, {
    ...
    .then((replyData) => {
      ...
      document.getElementById(`comments-${parent}`).querySelector('.flex-row.comment-body.column')
      ...
      const topLevelComment = document.getElementById(`comments-${parent}`).parentNode.classList.contains('replies')
        ? document.getElementById(`comments-${parent}`)
      ...
      if (!topLevelComment.parentNode.querySelector('.flex-row.replies.column')) {
        ...
        topLevelComment.parentNode
          .appendChild(replyColumn);
      }
...
function addReply (comment, commentData, replyData, replyUsernames, repliesData) {
  ...
  if (reply.querySelector('span.comment-delete')) {
    reply.querySelector('span.comment-delete')
      .addEventListener('click', () => {
  ...
  if (reply.querySelector('span.comment-report')) {
    reply.querySelector('span.comment-report')
      .addEventListener('click', () => {
        openModal('report', replyData.id);
      });
  }

  reply.querySelector('span.comment-reply')
    .addEventListener('click', () => {
  ...
  if (comment.querySelectorAll('.flex-row.comment').length === 4) {
    ...
    seeMore.addEventListener('click', () => {
      for (let i = 3; i < repliesData.length; i++)
        addReply(comment, commentData, repliesData[i], replyUsernames);
        ...
        for (const [ index, commentData ] of data.entries()) {
          ...
          <div class="flex-row comment" id="comments-${commentData.id}">
            ...
            if (comment.querySelector('span.comment-delete')) {
              comment.querySelector('span.comment-delete')
                .addEventListener('click', () => {
            ...
            if (comment.querySelector('span.comment-report')) {
              comment.querySelector('span.comment-report')
                .addEventListener('click', () => {
                  openModal('report', commentData.id);
                });
            }

            comment.querySelector('span.comment-reply')
              .addEventListener('click', () => {
                openReplyBox(commentData.id);
              });

            (document.querySelector('.flex-row.comments-list') || document.querySelector('.studio-compose-container').children[1]).insertBefore(comment, topComment);
          ...
          fetch(`${COMMENTS_API_URL}/${commentData.id}/replies`)
            ...
            .then((repliesData) => {
              ...
              for (const replyData of repliesData)
                replyUsernames[replyData.author.id] = replyData.author.username;

              for (const replyData of repliesData) {
                if (!document.getElementById(`comments-${commentData.id}`).parentNode.querySelector('.flex-row.replies.column')) {
                  const replyColumn = document.createElement('div');

                  replyColumn.classList.add('flex-row', 'replies', 'column');
                  comment.appendChild(replyColumn);
                }
                if (!document.getElementById(`comments-${replyData.id}`) && comment.querySelectorAll('.flex-row.comment').length < 4)
...
setInterval(() => {
  ...
  for (const [ id, commentTime ] of Object.entries(newComments))
    document.getElementById(`comments-${id}`).querySelector('.comment-time').childNodes[1].textContent = getRelativeTime(new Date(commentTime), currentTime);
}, 10000);
