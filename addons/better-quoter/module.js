...
function getSelectionBBCode(selection) {
    ...
    for (const quote of quotes) {
      ...
      quote.insertAdjacentText("afterbegin", "[quote" + (authorText ? `=${authorText.slice(0, authorText.length - 7)}]\n` : "]\n"));
...
function setup() {
  ...
  window.copy_paste = function (id) {
    ...
    const idText = !forumIdAddon?.self?.disabled && forumIdAddon?.settings?.get?.("auto_add") ? getIDLink(id.substring(1), post["0"].querySelector(".box-head > .conr").textContent, false) : "";
