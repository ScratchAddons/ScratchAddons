const regexDict = {
  project: [/^(?:(?:https?:)?\/\/scratch\.mit\.edu)?\/projects\/(\d+)/],
  studio: [/^(?:(?:https?:)?\/\/scratch\.mit\.edu)?\/studios\/(\d+)/],
  user: [/^(?:(?:https?:)?\/\/scratch\.mit\.edu)?\/users\/([a-zA-Z0-9-_]+)/],
  forumTopic: [/^(?:(?:https?:)?\/\/scratch\.mit\.edu)?\/discuss\/topic\/(\d+)(?!.+#post-\d)/],
  forumPost: [
...
const tooltipContentFunctions = {
  ...
  scratch2: {
    async project(msg, id) {
      ...
      wrapper.querySelector(".sa-hovercards-project-author").textContent = msg("info-by", {
  ...
  extended: {
    async project(msg, id) {
      ...
      infoExtendedWrapper.textContent =
  ...
  extendedStatistics: {
    ...
    async studio(msg, id) {
      ...
      projectsText.textContent =
        data.stats.projects === 100 ? msg("info-projects-100") : msg("info-projects", { count: data.stats.projects });
      commentsText.textContent =
        data.stats.comments === 100 ? msg("info-comments-100") : msg("info-comments", { count: data.stats.comments });
...
export default async function ({ addon, msg }) {
  ...
  const changeColorScheme = async () => {
    if (addon.settings.get("color-scheme") !== "auto") {
      ...
      if ((await addon.self.getEnabledAddons()).includes("dark-www"))
        tippyGlobalOptionsChanging.theme = "sa-hovercards-dark";
