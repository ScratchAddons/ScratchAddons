...
export default async function ({ addon, console }) {
  ...
  function createMessage(info) {
    ...
    let messageInfoValue = (commentInfo ? commentInfo : messageInfoContainer).appendChild(
    ...
    if (info.type == "addcomment") {
      ...
      messageInfoValue.appendChild(
        document.createTextNode(
          info.commentee_username == addon.auth.username ? " replied to your comment on " : " commented on "
        )
      );
      let actionLink = messageInfoValue.appendChild(document.createElement("a"));
      actionLink.innerText =
        info.comment_type == 1
          ? info.comment_obj_title == addon.auth.username
            ? "your profile"
            : info.comment_obj_title + "'s profile"
          : info.comment_obj_title;
      actionLink.href = `/${info.comment_type == 0 ? "projects" : info.comment_type == 1 ? "users" : "studios"}/${
        (info.comment_type == 1 ? info.comment_obj_title : info.comment_obj_id) +
        (info.comment_type == 2 ? "/comments" : "")
      }/#comments-${info.comment_id}/`;
    }
  ...
  async function getVaildComments(valid = [], offset = 0) {
    ...
    await addon
      .fetch(`https://api.scratch.mit.edu/users/${addon.auth.username}/messages?limit=40&offset=${offset * 40}`)
      .then((response) => {
      ...
      .then((messages) => {
        ...
        for (let i = 0; i < messages.length; i++) {
          ...
          if (messageType == "addcomment") {
            ...
            } else if (messages[i].comment_type === 1) {
              // Profile comment
