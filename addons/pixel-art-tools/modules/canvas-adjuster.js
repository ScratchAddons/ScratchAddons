...
export function createCanvasAdjuster(paper) {
  const getBgLayer = () =>
    paper.project.layers.find(l => l?.data?.isBackgroundGuideLayer);
  const getOutlineLayer = () =>
    paper.project.layers.find(l => l?.data?.isOutlineLayer);
  ...
  const makeChecker = (w, h, size) => {
    ...
    for (let y = 0; y < h; y += size) {
      for (let x = 0; x < w; x += size) {
        g.addChild(new paper.Path.Rectangle({
          point: [x, y],
          size,
