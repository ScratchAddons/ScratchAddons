export default async function ({ addon, console, safeMsg }) {
  ...
  function createAssetConflictDialog(fileName, actionClickCallback) {
        ...
        <div class = 'conflictDialog-content'>
          <div>
            <div class="conflictDialog-actions">
              <button id="rename" name="conflictAction" value="rename" class="conflictDialog-button selected">${safeMsg("rename")}</button>
              <button id="replace" name="conflictAction" value="replace" class="conflictDialog-button">${safeMsg("replace")}</button>
              <button id="skip" name="conflictAction" value="skip" class="conflictDialog-button">${safeMsg("skip")}</button>
            </div>
    ...
    // Create the modal
    const {remove, content, closeButton, container} = addon.tab.createModal('Rename, Replace or Skip Assets', {isOpen: true, useEditorClasses: true})
        ...
        const conflictHandler = (action)=>{
          switch(action){
            case 'rename':
              ...
              break;
            case 'replace':{
              resolve(originalFn.call(this, ...args).then(()=>{
