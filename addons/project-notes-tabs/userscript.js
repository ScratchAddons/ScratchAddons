export default async function ({ addon, global, console, msg }) {
  while (true) {
    ...
    if (allLabels - 1) {
      ...
      innerTab.innerText = msg("instructions");
    ...
    innerTab.innerText = msg("credits");
    ...
    for (var i = 0; i < tabs.querySelectorAll(".tab-choice-sa").length; i++) {
      tabs.querySelectorAll(".tab-choice-sa")[i].addEventListener("click", function (e) {
        selectTab((e.path[0].classList.length ? e.path[0].children[0] : e.path[0]).innerText === msg("instructions") ? 0 : 1);
      });
    }
    selectTab(0);
