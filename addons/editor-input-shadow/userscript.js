  ...
  const spawnAt = (xml, targetPos) => {
    ...
    if (myC && targetPos !== undefined) {
      const cfg = {
        ...
        get offX() {
          return -(this.rangeX / 2);
        },
        get offY() {
          return -(this.rangeY / 2);
        },
      };
  ...
  const input_lock = (block, opcodeData) => {
    ...
    try {
      ...
      Array.from(xml.childNodes)
        .filter((n) => n.tagName && n.tagName.toLowerCase() === "value")
        .forEach((v) => process(v, true, block.id));

      const parent = block.getParent();
      const pConn = parent
        ? parent.getConnections_().find((c) => c.targetConnection && c.targetConnection.sourceBlock_ === block)
        : null;
      const bConnType = pConn
        ? block.getConnections_().find((c) => c.targetConnection && c.targetConnection.sourceBlock_ === parent).type
        : null;
      block.dispose();
      ...
      if (pConn)
        newBlock
          .getConnections_()
          .find((c) => c.type === bConnType)
          .connect(pConn);
      spawnList.forEach((item) => spawnAt(item.xml, item.pos));
    } catch (e) {
      console.error(`[${addonname}]` + e);
    } finally {
  ...
  const process_all = (block, mode) => {
    try {
      ...
      const transform = (element, parentId) => {
        Array.from(element.querySelectorAll(":scope > value, :scope > statement, :scope > next")).forEach(
          (container) => {
    ...
    (items, block) => {
      if (!addon.self.disabled) {
        ...
        if (addon.settings.get("full_shadow_lock")) {
          ...
          items.push({
            enabled: true,
            text: msg("text-all-to-nonshadow"),
            callback: () => process_all(block, "UNLOCK"),
          });
        }
        return items;
      }
