        ...
        while (this.runtime.threads.length > 0 &&
            ...
            for (let i = startIdx; i < threads.length; i++) {
                ...
                if (activeThread.stack.length === 0 ||
                    activeThread.status === Thread.STATUS_DONE) { // Is thead finished
                    stoppedThread = true;
                    continue;
                }
                if (activeThread.status === Thread.STATUS_YIELD_TICK && // Was thead yeilding for only one tick?
                    !ranFirstTick) {
    ...
    stepThread(thread) {
        ...
        while ((currentBlockId = thread.peekStack())) {
            ...
            if (thread.status === Thread.STATUS_YIELD) {
                thread.status = Thread.STATUS_RUNNING;
                if (isWarpMode &&
                    thread.warpTimer.timeElapsed() <= WARP_TIME) {
                    continue;
                }
                return;
            } else if (thread.status === Thread.STATUS_PROMISE_WAIT) {
