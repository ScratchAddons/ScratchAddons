export default async function ({ addon, global, console }) {
  ...
  function make(display) {
    // function that creates the share button
  ...
  async function run(runs, elements) {
    // main script
    let not_shared = document.querySelectorAll(".media-item-content.not-shared .media-action div")[runs];
    if (!not_shared.hasAttribute("data-sa_mystuff_sharing_added")) not_shared.appendChild(make("Share"));
    not_shared.dataset.sa_mystuff_sharing_added = 1;
    if (runs + 1 !== elements) run(runs + 1, elements);
  }
  async function basics() {
    // setup for new runs of the script
