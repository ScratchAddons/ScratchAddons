...
import {
    updateAllBlocks
} from "../../libraries/common/cs/update-all-blocks.js";
...
export default async function({
    ...
    const secondaryColor = (category) => {
        ...
        if (isColoredTextMode())
            return alphaBlend(primaryColor(category), multiply(addon.settings.get(category.settingId), {
                a: 0.15
            }));
        if (textMode() === "black") return brighten(addon.settings.get(category.settingId), {
            r: 0.6,
            g: 0.6,
            b: 0.6
        });
        return multiply(addon.settings.get(category.settingId), {
            r: 0.9,
            g: 0.9,
            b: 0.9
        });
    ...
    const tertiaryColor = (category) => {
        ...
        if (textMode() === "black") return multiply(addon.settings.get(category.settingId), {
            r: 0.65,
            g: 0.65,
            b: 0.65
        });
    ...
    const fieldBackground = (category) => {
        ...
        if (category instanceof Blockly.Block || category instanceof Blockly.Field) {
            ...
            if (isColoredTextMode() || textMode() === "black") {
                ...
                if (isColoredTextMode()) return alphaBlend(primary, multiply(block.getColourTertiary(), {
                    a: 0.25
                }));
                else return brighten(primary, {
                    r: 0.4,
                    g: 0.4,
                    b: 0.4
                });
        ...
        if (textMode() === "black") return brighten(primaryColor(category), {
            r: 0.4,
            g: 0.4,
            b: 0.4
        });
    ...
    if (Blockly.registry) {
        ...
        Blockly.Field.prototype.getConstants = function() {
            ...
            for (const [name, settingId] of [
                    ["FIELD_BORDER_RECT_COLOUR", "input-color"]
                ]) {
