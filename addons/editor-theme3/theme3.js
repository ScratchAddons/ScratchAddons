...
export default async function ({ addon, console, msg }) {
  ...
  const secondaryColor = (category) => {
    ...
    if (isColoredTextMode())
      return alphaBlend(
        ...
        multiply(addon.settings.get(category.settingId), {
          a: 0.15,
        })
      );
    if (textMode() === "black")
      return brighten(addon.settings.get(category.settingId), {
        r: 0.6,
        g: 0.6,
        b: 0.6,
      });
  ...
  const fieldBackground = (category) => {
    ...
    if (category instanceof Blockly.Block || category instanceof Blockly.Field) {
      ...
      if (isColoredTextMode() || textMode() === "black") {
        ...
        if (isColoredTextMode())
          return alphaBlend(
            primary,
            multiply(block.getColourTertiary(), {
              a: 0.25,
            })
          );
        else
          return brighten(primary, {
            r: 0.4,
            g: 0.4,
            b: 0.4,
          });
    ...
    if (textMode() === "black")
      return brighten(primaryColor(category), {
        r: 0.4,
        g: 0.4,
        b: 0.4,
      });
