export default async function ({ addon, msg, global, console }) {
	...
	ScratchBlocks.BlockSvg.prototype.jsonInit = function(json) {
		if (
			!addon.self.disabled &&
			opcodeToSettings[this.type] && addon.settings.get(opcodeToSettings[this.type])
		) {
	...
	function updateAllBlocks() {
      ...
      if (workspace) {
        ...
        if (flyout) {
          ...
          ScratchBlocks.Xml.clearWorkspaceAndLoadFromXml(ScratchBlocks.Xml.workspaceToDom(flyoutWorkspace), flyoutWorkspace);
