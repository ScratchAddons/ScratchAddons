export default async function ({ addon, global, console }) {
  ...
  document.addEventListener("keydown", function (e) {
    for (var i = 0; i < allSettings.length; i++) {
      ...
      if (e.shiftKey == converted.shiftKey && e.ctrlKey && (converted.key < 10 && converted.key >= 0 ? (e.shiftKey ? e.code == "Digit" + converted.key : e.key == converted.key) : e.key == converted.key)) {
        e.preventDefault()
        if (i == 0) {
