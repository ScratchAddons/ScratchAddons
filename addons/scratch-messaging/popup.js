...
const vue = new Vue({
  ...
  methods: {
      ...
      return new Promise(resolve => {
        chrome.runtime.sendMessage({
          ...
          for(const commentId in comments) {
            ...
            const resourceGetFunction = resourceType === "project" ? "getProjectObject" : resourceType === "user" ? "getProfileObject" : "getStudioObject";
    ...
    async analyzeMessages(showAll = false) {
      ...
      const messagesToCheck = this.msgCount > 40 ? this.messages.length : showAll ? this.messages.length : this.msgCount;
      ...
      for (const indexString in this.messages.slice(0, messagesToCheck)) {
        ...
        if (message.type === "followuser") {
          ...
          else if(message.comment_type === 2) resourceObject = this.getStudioObject(resourceId, message.comment_obj_title);
