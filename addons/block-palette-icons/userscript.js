...
export default async function({addon, global, console}){
	while (true) {
		const tabs = await addon.tab.waitForElement(
			".scratchCategoryMenu", {
				markAsSeen: true
			})
		...
		function do_da_icons() {
			 ...
			 */
			let icons = ["ic:twotone-motion-photos-on",
				"ant-design:eye-filled",
				"fluent:speaker-1-24-filled", "typcn:warning",
				"entypo:switch", "jam:layers-f", "mdi:function",
				"fluent:number-symbol-16-filled",
				"clarity:block-line"
			];
            try {
                //For each .scratchCategoryItemBubble add an icon
                document.querySelectorAll(".scratchCategoryItemBubble")
                    .forEach((item, i) => {
                        ...
                        //Set the innerHTML to include the icon. (This is where we use the icons array)
                        item.innerHTML =
                            `<span class="iconify" style="top: 15%; color: white; left: 15%; position: absolute; font-size: 17px" data-icon="${icons[i]}"></span>`
                    })
            } catch(e) {
