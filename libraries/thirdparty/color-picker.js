 ...
 * subject to an additional IP rights grant found at
 * http://polymer.github.io/PATENTS.txt
 */"undefined"!=typeof window&&(window.litHtmlVersions||(window.litHtmlVersions=[])).push("1.4.0");const lt=(t,...e)=>new K(t,e,"html",at)
/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
      ...
      <div id="container">
        ...
        <section id="sliderInput">
          <div id="sliders">
            ...
            <color-picker-slider tabindex="0" .label="${"change alpha"}" id="alphaInput" class="absbefore absafter checkerboard" ?hidden="${this.no_alpha}" .value="${100*this.alpha}" min="0" max="100" step="1" @input="${this._handleAlphaSliderInput}" @change="${this._handleAlphaSliderInput}" @mousedown="${()=>this._sliderDown=!0}" @mouseup="${()=>this._sliderDown=!1}"></color-picker-slider>
          </div>
        ...
        <section id="textInput">
          ...
          <div ?hidden="${"hsv"!==this.selectedFormat}" class="color-input">
            ...
            <label ?hidden="${this.no_alpha}" data-name="%"><input aria-label="change alpha" type="number" .value="${Math.round(100*this.alpha)}" min="0" max="100" step="1" data-scheme="alpha" @input="${this._handleAlphaInput}"></label>
          </div>
