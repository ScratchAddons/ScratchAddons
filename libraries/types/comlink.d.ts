...
/**
 * Takes a type and wraps it in a Promise, if it not already is one. This is to avoid `Promise<Promise<T>>`.
 *
 * This is the inverse of `Unpromisify<T>`.
 */
declare type Promisify<T> = T extends Promise<unknown> ? T : Promise<T>;
/**
 * Takes a type that may be Promise and unwraps the Promise type. If `P` is not a Promise, it returns `P`.
 *
 ...
 */
export declare type Remote<T> = RemoteObject<T> &
 ...
 */
export declare type Local<T> = Omit<LocalObject<T>, keyof ProxyMethods> &
  (T extends (...args: infer TArguments) => infer TReturn
    ? (
        ...args: {
...
export interface TransferHandler<T, S> {
  /**
   * Gets called for every value to determine whether this transfer handler should serialize the value, which includes
   * checking that it is of the right type (but can perform checks beyond that as well).
   */
  canHandle(value: unknown): value is T;
  /**
   * Gets called with the value if `canHandle()` returned `true` to produce a value that can be sent in a message,
   * consisting of structured-cloneable values and/or transferrable objects.
   */
  serialize(value: T): [S, Transferable[]];
  /**
   * Gets called to deserialize an incoming value that was serialized in the other thread with this transfer handler
   * (known through the name it was registered under).
   */
