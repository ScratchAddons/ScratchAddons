/* Please note that this file has been formatted slightly differently from its original state. */
...
declare namespace chrome.action {
  ...
  export interface TabIconDetails {
    /**
     * Optional. Either a relative image path or a dictionary {size -> relative image path} pointing to icon to be set.
     * If the icon is specified as a dictionary, the actual image to be used is chosen depending on screen's pixel
     * density. If the number of image pixels that fit into one screen space unit equals scale, then image with size
     * scale * 19 will be selected. Initially only scales 1 and 2 will be supported. At least one image must be
    ...
    tabId?: number;
    /**
     * Optional. Either an ImageData object or a dictionary {size -> ImageData} representing icon to be set. If the icon
     * is specified as a dictionary, the actual image to be used is chosen depending on screen's pixel density. If the
     * number of image pixels that fit into one screen space unit equals scale, then image with size scale * 19 will be
     * selected. Initially only scales 1 and 2 will be supported. At least one image must be specified. Note that
     * 'details.imageData = foo' is equivalent to 'details.imageData = {'19': foo}'
     */
   ...
   *
   * @returns The `disable` method provides its result via callback or returned as a `Promise` (MV3 only). It has no parameters.
   */
   ...
   * @param tabId The id of the tab for which you want to modify the action.
   *
   * @returns The `enable` method provides its result via callback or returned as a `Promise` (MV3 only). It has no parameters.
   */
   ...
   * Gets the background color of the action.
   *
   * @returns The `getBadgeBackgroundColor` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   * DeclarativeNetRequestFeedback permission is present or tab-specific badge text was provided.
   *
   * @returns The `getBadgeText` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   * Gets the html document set as the popup for this action.
   *
   * @returns The `getPopup` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   * Gets the title of the action.
   *
   * @returns The `getTitle` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   *
   * @returns The `setBadgeBackgroundColor` method provides its result via callback or returned as a `Promise` (MV3
   *   only). It has no parameters.
   */
   ...
   *
   * @returns The `setBadgeText` method provides its result via callback or returned as a `Promise` (MV3 only). It has
   *   no parameters.
   */
   ...
   * Since Chrome 88.
   *
   * Sets the icon for the action. The icon can be specified either as the path to an image file or as the pixel data
   ...
   *
   * @returns The `setPopup` method provides its result via callback or returned as a `Promise` (MV3 only). It has no parameters.
   */
   ...
   *
   * @returns The `setTitle` method provides its result via callback or returned as a `Promise` (MV3 only). It has no parameters.
   */
...
declare namespace chrome.alarms {
  export interface AlarmCreateInfo {
    ...
    /**
     * Optional. If set, the onAlarm event should fire every periodInMinutes minutes after the initial event specified
     * by when or delayInMinutes. If not set, the alarm will only fire once.
     */
  ...
  export interface Alarm {
    ...
    periodInMinutes?: number;
    /**
     * Time at which this alarm was scheduled to fire, in milliseconds past the epoch (e.g. Date.now() + n). For
     * performance reasons, the alarm may have been delayed an arbitrary amount beyond this.
     */
   ...
   * delay them an arbitrary amount more. That is, setting delayInMinutes or periodInMinutes to less than 1 will not be
   * honored and will cause a warning. when can be set to less than 1 minute after "now" without warning but won't
   * actually cause the alarm to fire for at least 1 minute.
   *
   * To help you debug your app or extension, when you've loaded it unpacked, there's no limit to how often the alarm can fire.
   *
   * @param alarmInfo Describes when the alarm should fire. The initial time must be specified by either when or
   *   delayInMinutes (but not both). If periodInMinutes is set, the alarm will repeat every periodInMinutes minutes
   *   after the initial event. If neither when or delayInMinutes is set for a repeating alarm, periodInMinutes is used
   ...
   *
   * @returns The `clearAll` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
...
////////////////////
/**
 * Use the chrome.bookmarks API to create, organize, and otherwise manipulate bookmarks. Also see Override Pages, which
...
declare namespace chrome.bookmarks {
  ...
  export interface BookmarkTreeNode {
    ...
    /**
     * The unique identifier for the node. IDs are unique within the current profile, and they remain valid even after
     ...
     * Since Chrome 37.
     *
     * Indicates the reason why this node is unmodifiable. The managed value indicates that this node was configured by
     * the system administrator or by the custodian of a supervised user. Omitted if the node can be modified by the
   ...
   * @param query A string of words and quoted phrases that are matched against bookmark URLs and titles.
   *
   * @returns The `search` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   * matching all specified properties.
   *
   * @param query An object with one or more of the properties query, url, and title specified. Bookmarks matching all
   ...
   * Retrieves the entire Bookmarks hierarchy.
   *
   * @returns The `getTree` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   * @param numberOfItems The maximum number of items to return.
   *
   * @returns The `getRecent` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   *
   * @returns The `remove` method provides its result via callback or returned as a `Promise` (MV3 only). It has no parameters.
   */
   ...
   * Retrieves the children of the specified BookmarkTreeNode id.
   *
   * @returns The `getChildren` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   * @param id The ID of the root of the subtree to retrieve.
   *
   * @returns The `getSubTree` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
  export function getSubTree(id: string): Promise<BookmarkTreeNode[]>;
  /**
   * Recursively removes a bookmark folder.
   *
   * @returns The `removeTree` method provides its result via callback or returned as a `Promise` (MV3 only). It has no
   *   parameters.
   */
...
////////////////////
/**
 * Use browser actions to put icons in the main Google Chrome toolbar, to the right of the address bar. In addition to
 * its icon, a browser action can also have a tooltip, a badge, and a popup.
 *
 * Availability: Since Chrome 5.
 *
...
declare namespace chrome.browsingData {
  export interface OriginTypes {
    ...
    /** Optional. Extensions and packaged applications a user has installed (be *really* careful!). */
    extension?: boolean;
  ...
  export interface RemovalOptions {
     ...
     * Since Chrome 21.
     *
     * An object whose properties specify which origin types ought to be cleared. If this object isn't specified, it
     * defaults to clearing only "unprotected" origins. Please ensure that you really want to remove application data
     * before adding 'protectedWeb' or 'extensions'.
     */
    originTypes?: OriginTypes;
    /**
     * Optional. Remove data accumulated on or after this date, represented in milliseconds since the epoch (accessible
     * via the getTime method of the JavaScript Date object). If absent, defaults to 0 (which would remove all browsing data).
     */
    since?: number;
  }
  ...
  export interface DataTypeSet {
    ...
    downloads?: boolean;
    /**
     * Optional. The browser's cache. Note: when removing data, this clears the entire cache: it is not limited to the
     * range you specify.
     */
    cache?: boolean;
  ...
  export interface SettingsCallback {
    ...
    /**
     * All of the types will be present in the result, with values of true if they are both selected to be removed and
     * permitted to be removed, otherwise false.
     */
    dataToRemove: DataTypeSet;
    /**
     * All of the types will be present in the result, with values of true if they are permitted to be removed (e.g., by
     * enterprise policy) and false if not.
     */
   ...
   * Since Chrome 26.
   *
   * Reports which types of data are currently selected in the 'Clear browsing data' settings UI. Note: some of the data
   * types included in this API are not available in the settings UI, and some UI settings control more than one data
   * type listed here.
   *
   * @param callback The callback parameter should be a function that looks like this:
   *
...
////////////////////
/**
 * Use the commands API to add keyboard shortcuts that trigger actions in your extension, for example, an action to open
 * the browser action or send a command to the extension.
 *
 ...
 * JavaScript, and plugins. More generally speaking, content settings allow you to customize Chrome's behavior on a
 * per-site basis instead of globally.
 *
...
declare namespace chrome.contentSettings {
  ...
  export interface ClearDetails {
     ...
     * - Incognito_session_only: setting for incognito profile that can only be set during an incognito session and is
  ...
  export interface SetDetails {
    ...
    setting: any;
    /**
     * Optional. The pattern for the secondary URL. Defaults to matching all URLs. For details on the format of a
     * pattern, see Content Setting Patterns.
     */
  ...
  export interface GetDetails {
    /**
     * Optional. The secondary URL for which the content setting should be retrieved. Defaults to the primary URL. Note
     * that the meaning of a secondary URL depends on the content type, and not all content types use secondary URLs.
     */
    secondaryUrl?: string;
    ...
    incognito?: boolean;
    /**
     * The primary URL for which the content setting should be retrieved. Note that the meaning of a primary URL depends
     * on the content type.
     */
    primaryUrl: string;
  }
  ...
  export interface ContentSetting {
     ...
     *
     *   Parameter resourceIdentifiers: A list of resource identifiers for this content type, or undefined if this content
     *   type does not use resource identifiers.
     */
   ...
   * Default is ask.
   *
   * The primary URL is the URL of the document which requested location data. The secondary URL is the URL of the
   ...
   * Default is ask.
   *
   * The primary URL is the URL of the document which requested to toggle the fullscreen mode. The secondary URL is the
   * URL of the top-level frame (which may or may not differ from the requesting URL).
   */
...
declare namespace chrome.contextMenus {
  export interface OnClickData {
     ...
     * Since Chrome 35.
     *
     * The URL of the page where the menu item was clicked. This property is not set if the click occured in a context
     * where there is no current page, such as in a launcher context menu.
     */
    pageUrl: string;
    /**
     * Optional.
  ...
  export interface CreateProperties {
    /**
     * Optional. Lets you restrict the item to apply only to documents whose URL matches one of the given patterns.
     * (This applies to frames as well.) For details on the format of a pattern, see Match Patterns.
     */
    documentUrlPatterns?: string[];
    /**
     * Optional. The initial state of a checkbox or radio item: true for selected and false for unselected. Only one
     * radio item can be selected at a time in a given group of radio items.
     */
    checked?: boolean;
    /**
     * Optional. The text to be displayed in the item; this is required unless type is 'separator'. When the context is
     * 'selection', you can use %s within the string to show the selected text. For example, if this parameter's value
     * is "Translate '%s' to Pig Latin" and the user selects the word "cool", the context menu item for the selection is
    ...
    enabled?: boolean;
    /**
     * Optional. Similar to documentUrlPatterns, but lets you filter based on the src attribute of img/audio/video tags
     * and the href of anchor tags.
     */
    targetUrlPatterns?: string[];
    /**
     * Optional.
     *
     * A function that will be called back when the menu item is clicked. Event pages cannot use this; instead, they
     * should register a listener for chrome.contextMenus.onClicked.
     *
   ...
   *
   * @param callback Called when the item has been created in the browser. If there were any problems creating the item,
...
declare namespace chrome.cookies {
   ...
   * corresponds to a cookie set with 'SameSite=None', 'lax' to 'SameSite=Lax', and 'strict' to 'SameSite=Strict'.
  ...
  export interface Cookie {
    ...
    hostOnly: boolean;
    /**
     * Optional. The expiration date of the cookie as the number of seconds since the UNIX epoch. Not provided for
     * session cookies.
     */
  ...
  /**
   * Represents a cookie store in the browser. An incognito mode window, for instance, uses a separate cookie store from
  ...
  export interface SetDetails {
    ...
    name?: string;
    /**
     * The request-URI to associate with the setting of the cookie. This value can affect the default domain and path
     * values of the created cookie. If host permissions for this URL are not specified in the manifest file, the API
     * call will fail.
     */
    url: string;
    /**
     * Optional. The ID of the cookie store in which to set the cookie. By default, the cookie is set in the current
    ...
    value?: string;
    /**
     * Optional. The expiration date of the cookie as the number of seconds since the UNIX epoch. If omitted, the cookie
    ...
    secure?: boolean;
    /**
     * Optional. The cookie's same-site status. Defaults to "unspecified", i.e., if omitted, the cookie is set without
     * specifying a SameSite attribute.
     *
     * @since Chrome 51.
     */
    sameSite?: SameSiteStatus;
   ...
   * Lists all existing cookie stores.
   *
   * @returns The `getAllCookieStores` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   *
   *   Optional parameter cookie: Contains details about the cookie that's been set. If setting failed for any reason,
   *   this will be "null", and "chrome.runtime.lastError" will be set.
   */
   ...
   * of "overwrite" . Afterwards, a new cookie is written with the updated values, generating a second notification with
   * "cause" "explicit".
   */
...
////////////////////
/**
 * The chrome.debugger API serves as an alternate transport for Chrome's remote debugging protocol. Use chrome.debugger
 * to attach to one or more tabs to instrument network interaction, debug JavaScript, mutate the DOM and CSS, etc. Use
 * the Debuggee tabId to target tabs with sendCommand and route events by tabId from onEvent callbacks.
 *
...
declare module chrome {
  namespace _debugger {
    ...
    export interface Debuggee {
       ...
       * Since Chrome 27.
       *
       * The id of the extension which you intend to debug. Attaching to an extension background page is only possible
       * when 'silent-debugger-extension-api' flag is enabled on the target browser.
       */
      extensionId?: string;
      /**
       * Optional.
     ...
     *   documentation pages.
     * @param callback Called once the attach operation succeeds or fails. Callback receives no arguments. If the attach
     ...
     * @param callback Called once the detach operation succeeds or fails. Callback receives no arguments. If the detach
     *   fails, runtime.lastError will be set to the error message.
     *
     *   If you specify the callback parameter, it should be a function that looks like this:
     *
     ...
     * @param callback Response body. If an error occurs while posting the message, the callback will be called with no
...
declare namespace chrome.declarativeContent {
  export interface PageStateUrlDetails {
    /**
     * Optional. Matches if the host name of the URL contains a specified string. To test whether a host name component
     * has a prefix 'foo', use hostContains: '.foo'. This matches 'www.foobar.com' and 'foo.com', because an implicit
     * dot is added at the beginning of the host name. Similarly, hostContains can be used to match against component
     * suffix ('foo.') and to exactly match against components ('.foo.'). Suffix- and exact-matching for the last
     * components need to be done separately using hostSuffix, because no implicit dot is added at the end of the host name.
     */
    hostContains?: string;
    ...
    querySuffix?: string;
    /**
     * Optional. Matches if the URL (without fragment identifier) contains a specified string. Port numbers are stripped
     * from the URL if they match the default port number.
     */
    urlContains?: string;
    /**
     * Optional. Matches if the URL (without fragment identifier) is equal to a specified string. Port numbers are
     * stripped from the URL if they match the default port number.
     */
    urlEquals?: string;
    /**
     * Optional. Matches if the URL (without fragment identifier) matches a specified regular expression. Port numbers
     * are stripped from the URL if they match the default port number. The regular expressions use the RE2 syntax.
     */
    urlMatches?: string;
    /**
     * Optional. Matches if the URL without query segment and fragment identifier matches a specified regular
     * expression. Port numbers are stripped from the URL if they match the default port number. The regular expressions
     * use the RE2 syntax.
     */
    originAndPathMatches?: string;
    /**
     * Optional. Matches if the URL (without fragment identifier) starts with a specified string. Port numbers are
     * stripped from the URL if they match the default port number.
     */
    urlPrefix?: string;
    /**
     * Optional. Matches if the URL (without fragment identifier) ends with a specified string. Port numbers are
     * stripped from the URL if they match the default port number.
     */
    urlSuffix?: string;
    ...
    schemes?: string[];
    /**
     * Optional. Matches if the port of the URL is contained in any of the specified port lists. For example [80, 443,
  ...
  export class PageStateMatcherProperties {
    ...
    /**
     * Optional. Matches if all of the CSS selectors in the array match displayed elements in a frame with the same
     * origin as the page's main frame. All selectors in this array must be compound selectors to speed up matching.
     * Note that listing hundreds of CSS selectors or CSS selectors that match hundreds of times per page can still slow
     * down web sites.
     */
    css?: string[];
    /**
...
declare namespace chrome.desktopCapture {
   ...
   *
   *   Parameter streamId: An opaque string that can be passed to getUserMedia() API to generate media stream that
   *   corresponds to the source selected by the user. If user didn't select any source (i.e. canceled the prompt) then
   *   the callback is called with an empty streamId. The created streamId can be used only once and expires after a few
   ...
   *
   * @param sources Set of sources that should be shown to the user.
   * @param targetTab Optional tab for which the stream is created. If not specified then the resulting stream can be
   *   used only by the calling extension. The stream can only be used by frames in the given tab whose security origin
   *   matches tab.url.
   * @param callback The callback parameter should be a function that looks like this:
   *
   *   Function(string streamId) {...};
...
declare namespace chrome.devtools.inspectedWindow {
  ...
  export interface Resource {
     ...
     * @param commit True if the user has finished editing the resource, and the new content of the resource should be
     *   persisted; false if this is a minor change sent in progress of the user editing the resource.
  ...
  export interface ReloadOptions {
    /**
     * Optional. If specified, the string will override the value of the User-Agent HTTP header that's sent while
     * loading the resources of the inspected page. The string will also override the value of the navigator.userAgent
     * property that's returned to any scripts that are running within the inspected page.
     */
    userAgent?: string;
    /**
     * Optional. When true, the loader will ignore the cache for all inspected page resources loaded before the load
     * event is fired. The effect is similar to pressing Ctrl+Shift+R in the inspected window or within the Developer
     * Tools window.
     */
    ignoreCache?: boolean;
    /**
     * Optional. If specified, the script will be injected into every frame of the inspected page immediately upon load,
     * before any of the frame's scripts. The script will not be injected after subsequent reloads—for example, if the
     * user presses Ctrl+R.
     */
     ...
     * preprocessorerScript function should return a string to be compiled by Chrome in place of the input source. In
     * the case that the source is a DOM event handler, the returned source must compile to a single JS function.
     *
  ...
  export interface EvaluationExceptionInfo {
    ...
    description: string;
    /**
     * Set if the error occurred on the DevTools side before the expression is evaluated, contains the array of the
     * values that may be substituted into the description string to provide more information about the cause of the error.
     */
    details: any[];
  ...
  export interface EvalOptions {
    ...
    frameURL?: string;
    /**
     * Evaluate the expression in the context of the content script of the calling extension, provided that the content
     * script is already injected into the inspected page. If not, the expression is not evaluated and the callback is
     * invoked with the exception parameter set to an object that has the isError field set to true and the code field
     * set to E_NOTFOUND.
     */
    useContentScriptContext?: boolean;
    /**
     * Evaluate the expression in the context of a content script of an extension that matches the specified origin. If
     * given, contextSecurityOrigin overrides the 'true' setting on userContentScriptContext.
     */
...
declare namespace chrome.devtools.panels {
  ...
  export interface ExtensionPanel {
     ...
     *
     * @param iconPath Path to the icon of the button. The file should contain a 64x24-pixel image composed of two 32x24
     *   icons. The left icon is used when the button is inactive; the right icon is displayed when the button is pressed.
  ...
  export interface Button {
    /**
     * Updates the attributes of the button. If some of the arguments are omitted or null, the corresponding attributes
     * are not updated.
     *
  ...
  export interface ExtensionSidebarPane {
     ...
     *
     * @param jsonObject An object to be displayed in context of the inspected page. Evaluated in the context of the
     *   caller (API client).
     * @param rootTitle An optional title for the root of the expression tree.
    ...
    /**
     * Fired when the sidebar pane becomes hidden as a result of the user switching away from the panel that hosts the
     * sidebar pane.
     */
   ...
   *
   * @param callback A function that is called when the user clicks on a valid resource link in Developer Tools window.
...
declare namespace chrome.downloads {
  ...
  export interface DownloadOptions {
    ...
    body?: string;
    /**
     * Optional. Use a file-chooser to allow the user to select a filename regardless of whether filename is set or
     * already exists.
     */
    saveAs?: boolean;
    /** The URL to download. */
    url: string;
    /**
     * Optional. A file path relative to the Downloads directory to contain the downloaded file, possibly containing
     * subdirectories. Absolute paths, empty paths, and paths containing back-references ".." will cause an error.
     * onDeterminingFilename allows suggesting a filename after the file's MIME type and a tentative filename have been
     * determined.
     */
    filename?: string;
    /**
     * Optional. Extra HTTP headers to send with the request if the URL uses the HTTP[s] protocol. Each header is
     * represented as a dictionary containing the keys name and either value or binaryValue, restricted to those allowed
     * by XMLHttpRequest.
     */
  ...
  export interface DownloadItem {
     ...
     * chrome.downloads.search({}, function(items){items.forEach(function(item){if (item.endTime) console.log(new
     * Date(item.endTime))})})
     */
    endTime?: string;
     ...
     * automatically watch for file removal. Call search() in order to trigger the check for file existence. When the
     * existence check completes, if the file has been deleted, then an onChanged event will fire. Note that search()
     * does not wait for the existence check to finish before returning, so results from search() may not accurately
     * reflect the file system. Also, search() may be called as often as necessary, but will not check for file
     * existence any more frequently than once every 10 seconds.
     */
    exists: boolean;
    /**
     * Optional. The identifier for the extension that initiated this download if this download was initiated by an
     * extension. Does not change once it is set.
     */
    byExtensionId?: string;
    /**
     * Optional. The localized name of the extension that initiated this download if this download was initiated by an
     * extension. May change if the extension changes its name or if the user changes their locale.
     */
  ...
  export interface GetFileIconOptions {
    /**
     * Optional. * The size of the returned icon. The icon will be square with dimensions size * size pixels. The
     * default and largest size for the icon is 32x32 pixels. The only supported sizes are 16 and 32. It is an error to
     * specify any other size.
     */
    size?: number;
  }
  ...
  export interface DownloadQuery {
    /**
     * Optional. Set elements of this array to DownloadItem properties in order to sort search results. For example,
     * setting orderBy=['startTime'] sorts the DownloadItem by their start time in ascending order. To specify
    ...
    /**
     * Optional. This array of search terms limits results to DownloadItem whose filename or url contain all of the
    ...
    /**
     * Optional. The maximum number of matching DownloadItem returned. Defaults to 1000. Set to 0 in order to return all
     * matching DownloadItem. See search for how to page through results.
     */
    limit?: number;
  ...
  export interface DownloadFilenameSuggestion {
    /**
     * The DownloadItem's new target DownloadItem.filename, as a path relative to the user's default Downloads
     * directory, possibly containing subdirectories. Absolute paths, empty paths, and paths containing back-references
     * ".." will be ignored.
     */
    filename: string;
    /** Optional. The action to take if filename already exists. */
   ...
   * the id field. To page through a large number of items, set orderBy: ['-startTime'], set limit to the number of
  ...
  /**
   * Retrieve an icon for the specified download. For new downloads, file icons are available after the onCreated event
   * has been received. The image returned by this function while a download is in progress may be different from the
   * image returned after the download is complete. Icon retrieval is done by querying the underlying operating system
   * or toolkit depending on the platform. The icon that is returned will therefore depend on a number of factors
   * including state of the download, platform, registered file types and visual theme. If a file icon cannot be
  ...
  /**
   * Download a URL. If the URL uses the HTTP[S] protocol, then the request will include all cookies currently set for
   * its hostname. If both filename and saveAs are specified, then the Save As dialog will be displayed, pre-populated
   * with the specified filename. If the download started successfully, callback will be called with the new
   * DownloadItem's downloadId. If there was an error starting the download, then callback will be called with
   * downloadId=undefined and runtime.lastError will contain a descriptive string. The error strings are not guaranteed
   * to remain backwards compatible between releases. Extensions must not parse it.
   *
   ...
   * Requires the "downloads.open" permission in addition to the "downloads" permission. An onChanged event will fire
   ...
   * page/browser action popup). Does not automatically accept dangerous downloads. If the download is accepted, then an
   * onChanged event will fire, otherwise nothing will happen. When all the data is fetched into a temporary file and
   * either the download is not dangerous or the danger has been accepted, then the temporary file is renamed to the
   ...
   * then it must return true. If the listener neither calls suggest synchronously nor returns true, then suggest will
   * be called automatically. The DownloadItem will not complete until all listeners have called suggest. Listeners may
   * call suggest without any arguments in order to allow the download to use downloadItem.filename for its filename, or
   * pass a suggestion object to suggest in order to override the target filename. If more than one extension overrides
   * the filename, then the last extension installed whose listener passes a suggestion object to suggest wins. In order
   * to avoid confusion regarding which extension will win, users should not install extensions that may conflict. If
   * the download is initiated by download and the target filename is known before the MIME type and tentative filename
   * have been determined, pass filename to download instead.
   */
...
declare namespace chrome.enterprise.platformKeys {
  export interface Token {
     ...
     *
     * Static IDs are "user" and "system", referring to the platform's user-specific and the system-wide hardware token,
     * respectively. Any other tokens (with other identifiers) might be returned by enterprise.platformKeys.getTokens.
     */
    id: string;
    /**
     * Implements the WebCrypto's SubtleCrypto interface. The cryptographic operations, including key generation, are
     * hardware-backed.
     *
     * Only non-extractable RSASSA-PKCS1-V1_5 keys with modulusLength up to 2048 can be generated. Each key can be used
     * for signing data at most once.
     *
     * Keys generated on a specific Token cannot be used with any other Tokens, nor can they be used with
     * window.crypto.subtle. Equally, Key objects created with window.crypto.subtle cannot be used with this interface.
     */
   ...
   * "system". The system-wide token will be the same for all sessions on this device (device in the sense of e.g. a Chromebook).
   *
  ...
  /**
   * Challenges a hardware-backed Enterprise Machine Key and emits the response as part of a remote attestation
   * protocol. Only useful on Chrome OS and in conjunction with the Verified Access Web API which both issues challenges
   * and verifies responses. A successful verification by the Verified Access Web API is a strong signal of all of the following:
   *
   ...
   * - The current device state complies with enterprise device policy. For example, a policy may specify that the device
   *   must not be in developer mode.
   * - Any device identity emitted by the verification is tightly bound to the hardware of the current device.
   *
   * This function is highly restricted and will fail if the current device is not managed, the current user is not
   * managed, or if this operation has not explicitly been enabled for the caller by enterprise device policy. The
   * Enterprise Machine Key does not reside in the "system" token and is not accessible by any other API.
   *
   * @since Chrome 50.
   * @param challenge A challenge as emitted by the Verified Access Web API.
   * @param registerKey If set, the current Enterprise Machine Key is registered with the "system" token and
   *   relinquishes the Enterprise Machine Key role. The key can then be associated with a certificate and used like any
   *   other signing key. This key is 2048-bit RSA. Subsequent calls to this function will then generate a new
   *   Enterprise Machine Key. Since Chrome 59.
   ...
   * Only useful on Chrome OS and in conjunction with the Verified Access Web API which both issues challenges and
   * verifies responses. A successful verification by the Verified Access Web API is a strong signal of all of the following:
   *
   * - The current device is a legitimate Chrome OS device.
   * - The current device is managed by the domain specified during verification.
   * - The current signed-in user is managed by the domain specified during verification.
   * - The current device state complies with enterprise user policy. For example, a policy may specify that the device
   *   must not be in developer mode.
   * - The public key emitted by the verification is tightly bound to the hardware of the current device and to the
   *   current signed-in user.
   *
   * This function is highly restricted and will fail if the current device is not managed, the current user is not
   * managed, or if this operation has not explicitly been enabled for the caller by enterprise user policy. The
   * Enterprise User Key does not reside in the "user" token and is not accessible by any other API.
   *
   * @since Chrome 50.
   * @param challenge A challenge as emitted by the Verified Access Web API.
   * @param registerKey If set, the current Enterprise User Key is registered with the "user" token and relinquishes the
   *   Enterprise User Key role. The key can then be associated with a certificate and used like any other signing key.
...
////////////////////
/**
 * Use the <code>chrome.enterprise.networkingAttributes</code> API to read information about your current network. Note:
 * This API is only available to extensions force-installed by enterprise policy.
 *
 * Important: This API works only on Chrome OS.
 *
 * @since Chrome 85.
...
declare namespace chrome.enterprise.networkingAttributes {
  ...
  /**
   * Retrieves the network details of the device's default network. If the user is not affiliated or the device is not
   * connected to a network, runtime.lastError will be set with a failure reason.
   *
...
////////////////////
/**
 * The chrome.events namespace contains common types used by APIs dispatching events to notify you when something
 * interesting happens.
 *
...
declare namespace chrome.events {
  ...
  export interface UrlFilter {
     ...
     * stripped from the URL if they match the default port number. The regular expressions use the RE2 syntax.
     */
    urlMatches?: string;
    /** Optional. Matches if the path segment of the URL contains a specified string. */
    pathContains?: string;
     ...
     * Since Chrome 28.
     *
     * Matches if the URL without query segment and fragment identifier matches a specified regular expression. Port
     * numbers are stripped from the URL if they match the default port number. The regular expressions use the RE2 syntax.
     */
    originAndPathMatches?: string;
  }
 ...
 * messages between an extension and its content scripts or between extensions, as described in detail in Message Passing.
 *
 * Availability: Since Chrome 5.
 */
declare namespace chrome.extension {
  export interface FetchProperties {
    ...
    windowId?: number;
    /**
     * Optional. The type of view to get. If omitted, returns all views (including background pages and tabs). Valid
     * values: 'tab', 'notification', 'popup'.
     */
    type?: string;
  }
   ...
   *
   *   Parameter response: The JSON response object sent by the handler of the request. If an error occurs while
   *   connecting to the extension, the callback will be called with no arguments and runtime.lastError will be set to
   *   the error message.
   */
...
declare namespace chrome.fileBrowserHandler {
  export interface SelectionParams {
     ...
     * Optional.
     *
     * List of file extensions that the selected file can have. The list is also used to specify what files to be shown
     * in the select file dialog. Files with the listed extensions are only shown in the dialog. Extensions should not
...
declare namespace chrome.fileSystemProvider {
  ...
  export interface EntryMetadata {
    ...
    mimeType?: string;
    /**
     * Optional. Thumbnail image as a data URI in either PNG, JPEG or WEBP format, at most 32 KB in size. Optional, but
     * can be provided only when explicitly requested by the onGetMetadataRequested event.
     */
    thumbnail?: string;
  }
  ...
  export interface MountOptions {
    ...
    displayName: string;
    /**
     * Optional. Whether the file system supports operations which may change contents of the file system (such as
     * creating, deleting or writing to files).
     */
    writable?: boolean;
    /**
     * Optional.
  ...
  export interface NotificationOptions {
    ...
    recursive: boolean;
    /**
     * The type of the change which happened to the observed entry. If it is DELETED, then the observed entry will be
     * automatically removed from the list of observed entries.
     */
    changeType: string;
    /** Optional. List of changes to entries within the observed directory (including the entry itself) */
    changes?: NotificationChange[];
    /**
     * Optional. Tag for the notification. Required if the file system was mounted with the supportsNotifyTag option.
   ...
   * descriptive but doesn't have to be unique. The fileSystemId must not be an empty string.
   *
   ...
   * the providing extension can decide to perform unmounting if not requested (eg. in case of lost connection, or a file error).
   *
   * In case of an error, runtime.lastError will be set with a corresponding error code.
   *
   * @param callback A generic result callback to indicate success or failure.
   ...
   * even if the system was shutdown. The last tag can be obtained with getAll.
   *
   ...
   * notification. Eg. if the providing extension starts after a reboot, and the last registered notification's tag is
   * equal to "123", then it should call notify for all changes which happened since the change tagged as "123". It
   * cannot be an empty string.
   *
   * Not all providers are able to provide a tag, but if the file system has a changelog, then the tag can be eg. a
   * change number, or a revision number.
   *
   * Note that if a parent directory is removed, then all descendant entries are also removed, and if they are watched,
   * then the API must be notified about the fact. Also, if a directory is renamed, then all descendant entries are in
   * fact removed, as there is no entry under their original paths anymore.
   *
   ...
   * directory already exists. If recursive is true, then all of the missing directories on the directory path must be created.
   */
...
declare namespace chrome.fontSettings {
  ...
  export interface FontDetailsResult {
    ...
    levelOfControl: string;
    /**
     * The font ID. Rather than the literal font ID preference value, this may be the ID of the font that the system
     * resolves the preference value to. So, fontId can differ from the font passed to setFont, if, for example, the
     * font is not available on the system. The empty string signifies fallback to the global script font setting.
     */
    fontId: string;
  }
...
declare namespace chrome.gcm {
  export interface OutgoingMessage {
    ...
    /**
     * The ID of the message. It must be unique for each message in scope of the applications. See the Cloud Messaging
     * documentation for advice for picking and handling an ID.
     */
    messageId: string;
    /**
     * Optional. Time-to-live of the message in seconds. If it is not possible to send the message within that time, an
     * onSendError event will be raised. A time-to-live of 0 indicates that the message should be sent immediately or
     * fail if it's not possible. The maximum and a default value of time-to-live is 86400 seconds (1 day).
     */
    timeToLive?: number;
    /**
     * Message data to send to the server. Case-insensitive goog. and google, as well as case-sensitive collapse_key are
     * disallowed as key prefixes. Sum of all key/value pairs should not exceed gcm.MAX_MESSAGE_SIZE.
     */
    data: Object;
  }
   ...
   *
   * @param senderIds A list of server IDs that are allowed to send messages to the application. It should contain at
   *   least one and no more than 100 sender IDs.
   * @param callback Function called when registration completes. It should check runtime.lastError for error when
   ...
   *
   * @param callback A function called after the unregistration completes. Unregistration was successful if
   ...
   * @param callback A function called after the message is successfully queued for sending. runtime.lastError should be
...
////////////////////
/**
 * Use the chrome.history API to interact with the browser's record of visited pages. You can add, remove, and query for
 * URLs in the browser's history. To override the history page with your own version, see Override Pages.
 *
 * Availability: Since Chrome 5.
 *
 * Permissions: "history"
...
declare namespace chrome.i18n {
  ...
  export interface DetectedLanguage {
     ...
     * [kLanguageInfoTable]{@link https://src.chromium.org/viewvc/chrome/trunk/src/third_party/cld/languages/internal/languages.cc}.
     *
   ...
   * (''). If the format of the getMessage() call is wrong — for example, messageName is not a string or the
...
declare namespace chrome.identity {
  ...
  export interface ProfileDetails {
     ...
     * Optional.
     *
     * A status of the primary account signed into a profile whose ProfileUserInfo should be returned. Defaults to SYNC
     * account status.
     */
  ...
  export interface TokenDetails {
     ...
     * Optional.
     *
     * Fetching a token may require the user to sign-in to Chrome, or approve the application's requested scopes. If the
     * interactive flag is true, getAuthToken will prompt the user as necessary. When the flag is false or omitted,
  ...
  export interface UserInfo {
    ...
    email: string;
    /**
     * A unique identifier for the account. This ID will not change for the lifetime of the account. Empty if the user
     * is not signed in or (in M41+) the identity.email manifest permission is not specified.
     */
    id: string;
  }
  ...
  export interface WebAuthFlowOptions {
     ...
     *
     * Since some auth flows may immediately redirect to a result URL, launchWebAuthFlow hides its web view until the
     * first navigation either redirects to the final URL, or finishes loading a page meant to be displayed.
     *
     * If the interactive flag is true, the window will be displayed when a page load completes. If the flag is false or
     * omitted, launchWebAuthFlow will return with an error if the initial navigation does not complete the flow.
     */
    interactive?: boolean;
  }
   ...
   *
   * The Identity API caches access tokens in memory, so it's ok to call getAuthToken non-interactively any time a token
   * is required. The token cache automatically handles expiration.
   *
   * For a good user experience it is important interactive token requests are initiated by UI in your app explaining
   * what the authorization is for. Failing to do this will cause your users to get authorization requests, or Chrome
   * sign in screens if they are not signed in, with with no context. In particular, do not use getAuthToken
   ...
   *
   * This API is different from identity.getAccounts in two ways. The information returned is available offline, and it
   ...
   *
   * If an access token is discovered to be invalid, it should be passed to removeCachedAuthToken to remove it from the
   * cache. The app may then retrieve a fresh token with getAuthToken.
   *
   ...
   *
   * This method enables auth flows with non-Google identity providers by launching a web view and navigating it to the
   * first URL in the provider's auth flow. When the provider redirects to a URL matching the pattern
   * https://<app-id>.chromiumapp.org/*, the window will close, and the final redirect URL will be passed to the
   * callback function.
   *
   * For a good user experience it is important interactive auth flows are initiated by UI in your app explaining what
   * the authorization is for. Failing to do this will cause your users to get authorization requests with no context.
...
declare namespace chrome.idle {
   ...
   *
   * @param detectionIntervalInSeconds The system is considered idle if detectionIntervalInSeconds seconds have elapsed
  ...
  /**
   * Gets the time, in seconds, it takes until the screen is locked automatically while idle. Returns a zero duration if
   * the screen is never locked automatically. Currently supported on Chrome OS only.
   *
   * @param callback The callback parameter should be a function that looks like this:
   *
   *   Function(integer delay) {...};
   *
   *   Parameter delay: Time, in seconds, until the screen is locked automatically while idle. This is zero if the screen
   *   never locks automatically.
   */
...
declare namespace chrome.input.ime {
  ...
  export interface KeyboardEvent {
     ...
     * Optional.
     *
     * The deprecated HTML keyCode, which is system- and implementation-dependent numerical code signifying the
   ...
   *
   * @param callback Called when the operation completes with a boolean indicating if the text was accepted or not. On
  ...
  /**
   * Sends the key events. This function is expected to be used by virtual keyboards. When key(s) on a virtual keyboard
   * is pressed by a user, this function is used to propagate that event to the system.
   *
   * @since Chrome 33.
   * @param callback Called when the operation completes.
   *
...
declare namespace chrome.management {
  ...
  export interface ExtensionInfo {
     ...
     * Optional.
     *
     * A list of icon information. Note that this just reflects what was declared in the manifest, and the actual image
     * at that url may be larger or smaller than what was declared, so you might consider using explicit width and
     * height attributes on img tags referencing these images. See the manifest documentation on icons for more details.
     */
    icons?: IconInfo[];
    /**
  ...
  export interface UninstallOptions {
     ...
     * Optional.
     *
     * Whether or not a confirm-uninstall dialog should prompt the user. Defaults to false for self uninstalls. If an
     * extension uninstalls another extension, this parameter is ignored and the dialog is always shown.
     */
   ...
   *
   * @returns The `setEnabled` method provides its result via callback or returned as a `Promise` (MV3 only). It has no
   *   parameters.
   */
   ...
   *
   * @returns The `getPermissionWarningsById` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   *
   * @returns The `getPermissionWarningsByManifest` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   *
   * @returns The `launchApp` method provides its result via callback or returned as a `Promise` (MV3 only). It has no parameters.
   */
   ...
   * @param id This should be the id from an item of management.ExtensionInfo.
   *
   * @returns The `uninstall` method provides its result via callback or returned as a `Promise` (MV3 only). It has no parameters.
   */
   ...
   *
   * @since Chrome 39.
   * @returns The `getSelf` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   *
   * @since Chrome 26.
   * @returns The `uninstallSelf` method provides its result via callback or returned as a `Promise` (MV3 only). It has
   *   no parameters.
   */
   ...
   *
   * @since Chrome 37.
   * @returns The `createAppShortcut` method provides its result via callback or returned as a `Promise` (MV3 only). It
   *   has no parameters.
   */
   ...
   *   ExtensionInfo.availableLaunchTypes, because the available launch types vary on different platforms and configurations.
   *
   * @returns The `setLaunchType` method provides its result via callback or returned as a `Promise` (MV3 only). It has
   *   no parameters.
   */
   ...
   *
   * @returns The `generateAppForLink` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
...
declare namespace chrome.networking.config {
  export interface NetworkInfo {
    ...
    SSID?: string;
    /**
     * Optional. The basic service set identification (BSSID) uniquely identifying the basic service set. BSSID is
     * represented as a human readable, hex-encoded string with bytes separated by colons, e.g. 45:67:89:ab:cd:ef.
     */
    BSSID?: string;
   ...
   *
   * @param networks Network filters to set. Every NetworkInfo must either have the SSID or HexSSID set. Other fields
   *   will be ignored.
   * @param callback Called back when this operation is finished.
   *
   *   The callback parameter should be a function that looks like this:
   ...
   * portal. When the extension finishes its attempt, it must call finishAuthentication with the GUID received with this
   * event and the appropriate authentication result.
   */
...
declare namespace chrome.notifications {
  ...
  export interface NotificationOptions {
     ...
     *
     * URLs can be a data URL, a blob URL, or a URL relative to a resource within this extension's .crx file Required
     * for notifications.create method.
     */
    iconUrl?: string;
     ...
     * Optional.
     *
     * A URL to the app icon mask. URLs have the same restrictions as iconUrl. The app icon mask should be in alpha
     * channel, as only the alpha channel of the image will be considered.
     *
     * @since Chrome 38.
     */
   ...
   *   If it matches an existing notification, this method first clears that notification before proceeding with the
   *   create operation.
   *
...
declare namespace chrome.omnibox {
  ...
  export interface SuggestResult {
    ...
    content: string;
    /**
     * The text that is displayed in the URL dropdown. Can contain XML-style markup for styling. The supported tags are
     * 'url' (for a literal URL), 'match' (for highlighting text that matched what the user's query), and 'dim' (for dim
     * helper text). The styles can be nested, eg. dimmed match. You must escape the five predefined entities to display
...
////////////////////
/**
 * Use the chrome.pageAction API to put icons inside the address bar. Page actions represent actions that can be taken
 * on the current page, but that aren't applicable to all pages.
 *
...
declare namespace chrome.pageAction {
  ...
  export interface IconDetails {
     ...
     * Optional.
     *
     * Either a relative image path or a dictionary {size -> relative image path} pointing to icon to be set. If the
     * icon is specified as a dictionary, the actual image to be used is chosen depending on screen's pixel density. If
     * the number of image pixels that fit into one screen space unit equals scale, then image with size scale * 19 will
     * be selected. Initially only scales 1 and 2 will be supported. At least one image must be specified. Note that
     * 'details.path = foo' is equivalent to 'details.imageData = {'19': foo}'
     */
    path?: any;
  }
...
declare namespace chrome.permissions {
  export interface Permissions {
     ...
     * Optional.
     *
     * List of named permissions (does not include hosts or origins). Anything listed here must appear in the
     * optional_permissions list in the manifest.
     */
    permissions?: string[];
    /**
     * Optional.
     ...
     * optional_permissions list in the manifest. For example, if http://*.example.com/ or http://* appears in
     * optional_permissions, you can request an origin of http://help.example.com/. Any path is ignored.
     */
    origins?: string[];
  }
...
declare namespace chrome.platformKeys {
  export interface Match {
    ...
    /**
     * The KeyAlgorithm of the certified key. This contains algorithm parameters that are inherent to the key of the
     * certificate (e.g. the key length). Other parameters like the hash function used by the sign function are not included.
     */
  ...
  export interface ClientCertificateSelectRequestDetails {
    /**
     * This field is a list of the types of certificates requested, sorted in order of the server's preference. Only
     * certificates of a type contained in this list will be retrieved. If certificateTypes is the empty list, however,
     * certificates of any type will be returned.
     */
    certificateTypes: string[];
    /**
     * List of distinguished names of certificate authorities allowed by the server. Each entry must be a DER-encoded
  ...
  export interface ClientCertificateSelectDetails {
     ...
     * Optional.
     *
     * If given, the selectClientCertificates operates on this list. Otherwise, obtains the list of all certificates
     * from the platform's certificate stores that are available to this extensions. Entries that the extension doesn't
     * have permission for or which doesn't match the request, are removed.
     */
    clientCerts?: ArrayBuffer[];
    /**
     * If true, the filtered list is presented to the user to manually select a certificate and thereby granting the
     * extension access to the certificate(s) and key(s). Only the selected certificate(s) will be returned. If is
     * false, the list is reduced to all certificates that the extension has been granted access to (automatically or manually).
     */
    interactive: boolean;
  }
  ...
  export interface ServerCertificateVerificationDetails {
    /**
     * Each chain entry must be the DER encoding of a X.509 certificate, the first entry must be the server certificate
     * and each entry must certify the entry preceding it.
     */
  ...
  export interface ServerCertificateVerificationResult {
    /**
     * The result of the trust verification: true if trust for the given verification details could be established and
     * false if trust is rejected for any reason.
     */
    trusted: boolean;
    /**
     * If the trust verification failed, this array contains the errors reported by the underlying network layer.
     * Otherwise, this array is empty.
     *
     * Note: This list is meant for debugging only and may not contain all relevant errors. The errors returned may
     * change in future revisions of this API, and are not guaranteed to be forwards or backwards compatible.
     */
    debug_errors: string[];
  }
   ...
   * user is presented a dialog where they can select from matching certificates and grant the extension access to the
   ...
   *
   *   Parameter matches: The list of certificates that match the request, that the extension has permission for and, if
   *   interactive is true, that were selected by the user.
   */
   ...
   * @param parameters Determines signature/hash algorithm parameters additionally to the parameters fixed by the key
   *   itself. The same parameters are accepted as by WebCrypto's importKey function, e.g. RsaHashedImportParams for a
   *   RSASSA-PKCS1-v1_5 key. For RSASSA-PKCS1-v1_5 keys, additionally the parameters { 'hash': { 'name': 'none' } } are
   *   supported. The sign function will then apply PKCS#1 v1.5 padding and but not hash the given data.
   ...
   *
   * @since Chrome 85.
   * @param publicKeySpkiDer A DER-encoded X.509 SubjectPublicKeyInfo, obtained e.g. by calling WebCrypto's exportKey
   *   function with format="spki".
   * @param parameters Provides signature and hash algorithm parameters, in addition to those fixed by the key itself.
   *   The same parameters are accepted as by WebCrypto's importKey function, e.g. RsaHashedImportParams for a
   *   RSASSA-PKCS1-v1_5 key. For RSASSA-PKCS1-v1_5 keys, we need to also pass a "hash" parameter { "hash": { "name":
   *   string } }. The "hash" parameter represents the name of the hashing algorithm to be used in the digest operation
   *   before a sign. It is possible to pass "none" as the hash name, in which case the sign function will apply PKCS#1
   *   v1.5 padding and but not hash the given data.
   *
   *   Currently, this function only supports the "RSASSA-PKCS1-v1_5" algorithm with one of the hashing algorithms "none",
   ...
   * by a known CA. The implementation is supposed to respect the EKU serverAuth and to support subject alternative names.
   *
   * @param callback The callback parameter should be a function that looks like this:
   *
   *   Function(object result) {...};
...
declare namespace chrome.power {
   ...
   * should be disabled. If a request previously made by the same app is still active, it will be replaced by the new request.
   */
...
////////////////////
/**
 * The chrome.printerProvider API exposes events used by print manager to query printers controlled by extensions, to
 * query their capabilities and to submit print jobs to these printers.
 *
...
declare namespace chrome.printerProvider {
   ...
   *
   * Note: An application should not rely on this event being fired more than once per device. If a connected device is
   * supported it should be returned in the onGetPrintersRequested event.
   *
   * @since Chrome 45.
   */
 ...
 * relies on the ChromeSetting prototype of the type API for getting and setting Chrome's configuration.
 *
 * Permissions: "privacy"
 *
...
declare namespace chrome.privacy {
  export interface Services {
    ...
    /**
     * @deprecated Since Chrome 70. Please use privacy.services.autofillAddressEnabled and
...
declare namespace chrome.proxy {
  ...
  export interface PacScript {
    ...
    url?: string;
    /**
     * Optional. If true, an invalid PAC script will prevent the network stack from falling back to direct connections.
     * Defaults to false.
     */
    mandatory?: boolean;
  ...
  /**
   * An object encapsulating the set of proxy rules for all protocols. Use either 'singleProxy' or (a subset of)
...
declare namespace chrome.serial {
  ...
  export interface DeviceInfo {
    /**
     * The device's system path. This should be passed as the path argument to chrome.serial.connect in order to connect
     * to this device.
     */
    path: string;
  ...
  export interface ConnectionInfo {
     ...
     *
     * This field may be omitted or inaccurate if a non-standard bitrate is in use, or if an error occurred while
     * querying the underlying device.
     */
    bitrate?: number;
    /**
     * Optional. See ConnectionOptions.dataBits. This field may be omitted if an error occurred while querying the
     * underlying device.
     */
    dataBits?: typeof DataBits[keyof typeof DataBits];
    /**
     * Optional. See ConnectionOptions.parityBit. This field may be omitted if an error occurred while querying the
     * underlying device.
     */
    parityBit?: typeof ParityBit[keyof typeof ParityBit];
    /**
     * Optional. See ConnectionOptions.stopBits. This field may be omitted if an error occurred while querying the
     * underlying device.
     */
  ...
  export interface ConnectionOptions {
    /**
     * Optional. Flag indicating whether or not the connection should be left open when the application is suspended
     * (see Manage App Lifecycle: https://developer.chrome.com/apps/app_lifecycle).
     *
     * The default value is "false." When the application is loaded, any serial connections previously opened with
     * persistent=true can be fetched with getConnections.
     */
    peristent?: boolean;
     ...
     *
     * There is no guarantee, of course, that the device connected to the serial port will support the requested
    ...
    ctsFlowControl?: boolean;
    /**
     * Optional. The maximum amount of time (in milliseconds) to wait for new data before raising an onReceiveError
    ...
    receiveTimeout?: number;
    /**
     * Optional. The maximum amount of time (in milliseconds) to wait for a send operation to complete before calling
     * the callback with a "timeout" error.
     *
...
declare namespace chrome.serial.onReceiveError {
  ...
  /**
   * Event raised when an error occurred while the runtime was waiting for data on the serial port.
   *
...
declare namespace chrome.runtime {
  ...
  export interface MessageSender {
    ...
    id?: string;
    /**
     * The tabs.Tab which opened the connection, if any. This property will only be present when the connection was
     * opened from a tab (including content scripts), and only if the receiver is an extension, not an app.
     */
    tab?: chrome.tabs.Tab;
    /**
    ...
    /**
     * The frame that opened the connection. 0 for top-level frames, positive for child frames. This will only be set
     * when tab is set.
     *
     * @since Chrome 41.
     */
    frameId?: number;
    /**
     * The URL of the page or frame that opened the connection. If the sender is in an iframe, it will be iframe's URL
     * not the URL of the page which hosts it.
     *
     * @since Chrome 28.
     */
    url?: string;
    ...
    tlsChannelId?: string;
    /**
     * The origin of the page or frame that opened the connection. It can vary from the url property (e.g., about:blank)
     * or can be opaque (e.g., sandboxed iframes). This is useful for identifying if the origin can be trusted if we
   ...
   *
   *   The ID of the extension or app to connect to. If omitted, a connection will be attempted with your own extension.
   ...
   * runtime.onMessage event will be fired in each page, or runtime.onMessageExternal, if a different extension. Note
   * that extensions cannot send messages to content scripts using this method. To send messages to content scripts, use
   * tabs.sendMessage.
   *
   * @since Chrome 26.
   * @param responseCallback Optional.
   *
   *   Parameter response: The JSON response object sent by the handler of the message. If an error occurs while
   *   connecting to the extension, the callback will be called with no arguments and runtime.lastError will be set to
   *   the error message.
   */
   ...
   *
   * @since Chrome 26.
   * @param extensionId The ID of the extension/app to send the message to. If omitted, the message will be sent to your
   *   own extension/app. Required if sending messages from a web page for web messaging.
   * @param responseCallback Optional.
   *
   *   Parameter response: The JSON response object sent by the handler of the message. If an error occurs while
   *   connecting to the extension, the callback will be called with no arguments and runtime.lastError will be set to
   ...
   * @param responseCallback Optional.
   *
   *   Parameter response: The response message sent by the native messaging host. If an error occurs while connecting to
   *   the native messaging host, the callback will be called with no arguments and runtime.lastError will be set to the
   *   error message.
   */
   ...
   *
   *   URL to be opened after the extension is uninstalled. This URL must have an http: or https: scheme. Set an empty
...
declare namespace chrome.scripting {
   ...
   *
   * @returns The `executeScript` method provides its result via callback or returned as a `Promise` (MV3 only). The
   *   resulting array contains the result of execution for each frame where the injection succeeded.
   */
   ...
   *
   * @param injection The details of the script which to inject.
   * @param callback Invoked upon completion of the injection. The resulting array contains the result of execution for
   *   each frame where the injection succeeded.
   */
   ...
   *
   * @returns The `insertCSS` method provides its result via callback or returned as a `Promise` (MV3 only). It has no parameters.
   */
...
declare namespace chrome.sessions {
   ...
   *
   * @param callback Parameter sessions: The list of closed entries in reverse order that they were closed (the most
   *   recently closed tab or window will be at index 0). The entries may contain either tabs or windows.
   */
   ...
   *
   * @param callback Parameter devices: The list of sessions.Device objects for each synced session, sorted in order
   *   from device with most recently modified session to device with least recently modified session. tabs.Tab objects
   *   are sorted by recency in the windows.Window of the sessions.Session objects.
   */
   ...
   *
   *   The windows.Window.sessionId, or tabs.Tab.sessionId to restore. If this parameter is not specified, the most
   *   recently closed session is restored.
...
declare namespace chrome.storage {
  export interface StorageArea {
     ...
     *
     * @param callback Callback with the amount of space being used by storage, or on failure (in which case
     ...
     * Gets the amount of space (in bytes) being used by one or more items.
     *
     * @param keys A single key or list of keys to get the total usage for. An empty list will return 0. Pass in null to
     *   get the total usage of all of storage.
     * @param callback Callback with the amount of space being used by storage, or on failure (in which case
     *   runtime.lastError will be set).
     *
     *   Parameter bytesInUse: Amount of space being used in storage, in bytes.
     ...
     *   typically serialize to {}, with the exception of Array (serializes as expected), Date, and Regex (serialize
  ...
  export interface LocalStorageArea extends StorageArea {
    /**
     * The maximum amount (in bytes) of data that can be stored in local storage, as measured by the JSON
     * stringification of every value plus every key's length. This value will be ignored if the extension has the
     * unlimitedStorage permission. Updates that would cause this limit to be exceeded fail immediately and set
     * runtime.lastError.
     */
    QUOTA_BYTES: number;
  }
  ...
  export interface SyncStorageArea extends StorageArea {
    ...
    /**
     * The maximum total amount (in bytes) of data that can be stored in sync storage, as measured by the JSON
     * stringification of every value plus every key's length. Updates that would cause this limit to be exceeded fail
     * immediately and set runtime.lastError.
     */
    QUOTA_BYTES: number;
    /**
     * The maximum size (in bytes) of each individual item in sync storage, as measured by the JSON stringification of
     * its value plus its key length. Updates containing items larger than this limit will fail immediately and set
     * runtime.lastError.
     */
    QUOTA_BYTES_PER_ITEM: number;
    /**
     * The maximum number of items that can be stored in sync storage. Updates that would cause this limit to be
     * exceeded will fail immediately and set runtime.lastError.
     */
    MAX_ITEMS: number;
    /**
     * The maximum number of set, remove, or clear operations that can be performed each hour. This is 1 every 2
     * seconds, a lower ceiling than the short term higher writes-per-minute limit.
     *
     * Updates that would cause this limit to be exceeded fail immediately and set runtime.lastError.
     */
    MAX_WRITE_OPERATIONS_PER_HOUR: number;
    /**
     * The maximum number of set, remove, or clear operations that can be performed each minute. This is 2 per second,
     * providing higher throughput than writes-per-hour over a shorter period of time.
     *
     * Updates that would cause this limit to be exceeded fail immediately and set runtime.lastError.
     *
     * @since Chrome 40.
...
declare namespace chrome.system.cpu {
   ...
   * Queries basic CPU information of the system.
   *
   * @returns The `getInfo` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
...
////////////////////
/**
 * Use the chrome.system.storage API to query storage device information and be notified when a removable storage device
...
declare namespace chrome.system.storage {
  export interface StorageUnitInfo {
    /**
     * The transient ID that uniquely identifies the storage device. This ID will be persistent within the same run of a
     * single application. It will not be a persistent identifier between different runs of an application, or between
   ...
   *
   * @param callback Parameter result: success: The ejection command is successful -- the application can prompt the
   *   user to remove the device; in_use: The device is in use by another application. The ejection did not succeed; the
   *   user should not remove the device until the other application is done with the device; no_such_device: There is
   *   no such device known. failure: The ejection command failed.
   */
   ...
   *   no such device known. failure: The ejection command failed.
   *
   * @returns The `ejectDevice` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   *
   * @since Dev channel only.
   * @returns The `getAvailableCapacity` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
...
declare namespace chrome.system.display {
  ...
  export interface DisplayInfo {
    ...
    dpiY: number;
    /**
     * The display's clockwise rotation in degrees relative to the vertical position. Currently exposed only on
     * ChromeOS. Will be set to 0 on other platforms.
     */
    rotation: number;
    /** The display's logical bounds. */
    bounds: Bounds;
    /**
     * The display's insets within its screen's bounds. Currently exposed only on ChromeOS. Will be set to empty insets
     * on other platforms.
     */
    overscan: Insets;
    /**
     * The usable work area of the display within the display bounds. The work area excludes areas of the display
     * reserved for OS, for example taskbar and launcher.
     */
    workArea: Bounds;
    /**
     * Requires(CrOS) Only available on Chrome OS.
  ...
  export interface MirrorModeInfo {
     ...
     * **mixed**
     *
     * The specified source display will be mirrored to the provided destination displays. All other connected displays
     * will be extended.
     */
   ...
   *
   * @since Chrome 53.
   * @returns The getDisplayLayout method provides its result via callback or returned as a Promise (MV3 only).
   */
   ...
   * @param info The information about display properties that should be changed. A property will be changed only if a
   *   new value for it is specified in |info|.
   *
   * @returns The `setDisplayProperties` method provides its result via callback or returned as a `Promise` (MV3 only).
   *   It has no parameters. To find out whether the function succeeded, runtime.lastError should be queried.
   */
   ...
   * @param info The information about display properties that should be changed. A property will be changed only if a
   *   new value for it is specified in |info|.
   * @param [callback] Empty function called when the function finishes. To find out whether the function succeeded,
   ...
   *
   * @returns The `setDisplayLayout` method provides its result via callback or returned as a `Promise` (MV3 only). It
   *   has no parameters. To find out whether the function succeeded, runtime.lastError should be queried.
   */
   ...
   * @since Chrome 57.
   * @param id The display's unique identifier.
   * @param callback Optional callback to inform the caller that the touch calibration has ended. The argument of the
   ...
   * @param id The display's unique identifier.
   *
   * @returns The `showNativeTouchCalibration` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   * @param bounds Bounds of the display when the touch calibration was performed. |bounds.left| and |bounds.top| values
   *   are ignored.
   * @throws Error.
   */
  ...
  /**
   * Resets the touch calibration for the display and brings it back to its default state by clearing any touch
   * calibration data associated with the display.
   *
   * @since Chrome 57.
   * @param id The display's unique identifier.
   */
   ...
   *
   * @since Chrome 65.
   * @returns The `setMirrorMode` method provides its result via callback or returned as a `Promise` (MV3 only). It has
   *   no parameters.
   */
...
declare namespace chrome.tabCapture {
  ...
  export interface GetMediaStreamOptions {
    /**
     * Optional tab id of the tab which will later invoke getUserMedia() to consume the stream. If not specified then
     * the resulting stream can be used only by the calling extension. The stream can only be used by frames in the
     * given tab whose security origin matches the consumber tab's origin. The tab's origin must be a secure origin, e.g. HTTPS.
     */
     ...
     * Only tabs for which the extension has been granted the activeTab permission can be used as the target tab.
     */
    targetTabId?: number;
  }
  ...
  /**
   * Creates a stream ID to capture the target tab. Similar to chrome.tabCapture.capture() method, but returns a media
   * stream ID, instead of a media stream, to the consumer tab.
   *
   * @param options Options for the media stream id to retrieve.
   * @param callback Callback to invoke with the result. If successful, the result is an opaque string that can be
   *   passed to the getUserMedia() API to generate a media stream that corresponds to the target tab. The created
   *   streamId can only be used once and expires after a few seconds if it is not used.
   */
 ...
 *
 * Permissions: The majority of the chrome.tabs API can be used without declaring any permission. However, the "tabs"
 * permission is required in order to populate the url, title, and favIconUrl properties of Tab.
 *
 * @since Chrome 5.
 */
declare namespace chrome.tabs {
  ...
  export interface MutedInfo {
    /**
     * Whether the tab is prevented from playing sound (but hasn't necessarily recently produced sound). Equivalent to
     ...
     * Optional.
     *
     * The ID of the extension that changed the muted state. Not set if an extension was not the reason the muted state
     * last changed.
     */
    extensionId?: string;
  }
  ...
  export interface Tab {
     ...
     * assigned an ID, for example when querying foreign tabs using the sessions API, in which case a session ID may be
     * present. Tab ID can also be set to chrome.tabs.TAB_ID_NONE for apps and devtools windows.
     */
    id?: number;
    ...
    audible?: boolean;
    /**
     * Whether the tab is discarded. A discarded tab is one whose content has been unloaded from memory, but is still
     * visible in the tab strip. Its content gets reloaded the next time it's activated.
     *
     * @since Chrome 54.
     */
    discarded: boolean;
  ...
  export interface ZoomSettings {
     ...
     *
     * "manual": Overrides the automatic handling of zoom changes. The onZoomChange event will still be dispatched, and
     * it is the responsibility of the extension to listen for this event and manually scale the page. This mode does
     * not support per-origin zooming, and will thus ignore the scope zoom setting and assume per-tab.
     *
     * "disabled": Disables all zooming in the tab. The tab will revert to the default zoom level, and all attempted
     * zoom changes will be ignored.
     */
    mode?: string;
    /**
     * Optional.
     ...
     *
     * "per-origin": Zoom changes will persist in the zoomed page's origin, i.e. all other tabs navigated to that same
     * origin will be zoomed as well. Moreover, per-origin zoom changes are saved with the origin, meaning that when
     * navigating to other pages in the same origin, they will all be zoomed to the same zoom factor. The per-origin
     * scope is only available in the automatic mode.
     *
     * "per-tab": Zoom changes only take effect in this tab, and zoom changes in other tabs will not affect the zooming
     * of this tab. Also, per-tab zoom changes are reset on navigation; navigating a tab will always load pages with
  ...
  export interface InjectDetails {
     ...
     * Optional.
     *
     * If allFrames is true, implies that the JavaScript or CSS should be injected into all frames of current page. By
     ...
     * extension has access to its parent document. Code cannot be inserted in top-level about:-frames. By default it is false.
     *
     * @since Chrome 39.
     */
  ...
  export interface CreateProperties {
     ...
     * windows.update). Defaults to true.
     *
     * @since Chrome 16.
     */
    active?: boolean;
  ...
  export interface CaptureVisibleTabOptions {
     ...
     * Optional.
     *
     * When format is "jpeg", controls the quality of the resulting image. This value is ignored for PNG images. As
     * quality is decreased, the resulting image will have more visual artifacts, and the number of bytes needed to
  ...
  export interface QueryInfo {
     ...
     * Optional.
     *
     * Whether the tabs are discarded. A discarded tab is one whose content has been unloaded from memory, but is still
     * visible in the tab strip. Its content gets reloaded the next time it's activated.
     *
     * @since Chrome 54.
     */
   ...
   *
   * @param details Details of the script or CSS to inject. Either the code or the file property must be set, but both
   ...
   *
   * @deprecated Since Chrome 33. Please use tabs.query {windowId: windowId}.
   * @returns The `getAllInWindow` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   *
   * @deprecated Since Chrome 33. Please use tabs.query {active: true}.
   * @returns The `getSelected` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   * Creates a new tab.
   *
   * @returns The `create` method provides its result via callback or returned as a `Promise` (MV3 only). Details about
   *   the created tab. Will contain the ID of the new tab.
   */
   ...
   *
   * @returns The `move` method provides its result via callback or returned as a `Promise` (MV3 only). Details about
   *   the moved tab.
   */
   ...
   *
   * @returns The `update` method provides its result via callback or returned as a `Promise` (MV3 only). Details about
   *   the updated tab. The tabs.Tab object doesn't contain url, title and favIconUrl if the "tabs" permission has not
   *   been requested.
   */
   ...
   * @param callback Optional.
   *
   *   Optional parameter tab: Details about the updated tab. The tabs.Tab object doesn't contain url, title and
  ...
  /**
   * Captures the visible area of the currently active tab in the specified window. You must have <all_urls> permission
   * to use this method.
   *
   * @param callback Parameter dataUrl: A data URL which encodes an image of the visible area of the captured tab. May
   *   be assigned to the 'src' property of an HTML Image element for display.
   */
   ...
   * to use this method.
   *
   * @returns The `captureVisibleTab` method provides its result via callback or returned as a `Promise` (MV3 only). A
   *   data URL which encodes an image of the visible area of the captured tab. May be assigned to the 'src' property of
   *   an HTML Image element for display.
   */
   ...
   *
   * @returns The `reload` method provides its result via callback or returned as a `Promise` (MV3 only). It has no parameters.
   */
   ...
   * @param callback Optional.
   *
   *   Optional parameter tab: Details about the duplicated tab. The tabs.Tab object doesn't contain url, title and
   *   favIconUrl if the "tabs" permission has not been requested.
   */
  export function duplicate(tabId: number, callback?: (tab?: Tab) => void): void;
  /**
   * Sends a single message to the content script(s) in the specified tab, with an optional callback to run when a
   * response is sent back. The runtime.onMessage event is fired in each content script running in the specified tab for
   ...
   *
   *   Parameter response: The JSON response object sent by the handler of the message. If an error occurs while
   *   connecting to the specified tab, the callback will be called with no arguments and runtime.lastError will be set
   ...
   * response is sent back. The extension.onRequest event is fired in each content script running in the specified tab
   ...
   *
   * @since Chrome 16.
   * @returns The `highlight` method provides its result via callback or returned as a `Promise` (MV3 only). Contains
   ...
   *
   * @param callback Parameter language: An ISO language code such as en or fr. For a complete list of languages
   *   supported by this method, see kLanguageInfoTable. The 2nd to 4th columns will be checked and the first non-NULL
   *   value will be returned except for Simplified Chinese for which zh-CN will be returned. For an unknown language,
   *   und will be returned.
   */
  export function detectLanguage(callback: (language: string) => void): void;
  /**
   * Detects the primary language of the content in a tab.
   *
   * @returns The `detectLanguage` method provides its result via callback or returned as a `Promise` (MV3 only). An ISO
   *   language code such as en or fr. For a complete list of languages supported by this method, see
   *   kLanguageInfoTable. The 2nd to 4th columns will be checked and the first non-NULL value will be returned except
   *   for Simplified Chinese for which zh-CN will be returned. For an unknown language, und will be returned.
   */
   ...
   *
   * @returns The `setZoom` method provides its result via callback or returned as a `Promise` (MV3 only). It has no parameters.
   */
   ...
   *
   * @since Chrome 42.
   * @returns The `getZoom` method provides its result via callback or returned as a `Promise` (MV3 only). The tab's
   *   current zoom factor.
   */
   ...
   *
   * @since Chrome 42.
   * @param tabId Optional. The ID of the tab to get the current zoom factor from; defaults to the active tab of the
   *   current window.
   * @param callback Called with the tab's current zoom factor after it has been fetched.
   *
   *   Parameter zoomFactor: The tab's current zoom factor.
   ...
   *
   * @returns The `setZoomSettings` method provides its result via callback or returned as a `Promise` (MV3 only). It
   *   has no parameters.
   */
   ...
   *
   * @since Chrome 42.
   * @returns The `getZoomSettings` method provides its result via callback or returned as a `Promise` (MV3 only). The
   ...
   *
   * @since Chrome 54.
   * @param tabId Optional. The ID of the tab to be discarded. If specified, the tab will be discarded unless it's
   *   active or already discarded. If omitted, the browser will discard the least important tab. This can fail if no
   ...
   *
   * @since Chrome 72.
   * @returns The `goForward` method provides its result via callback or returned as a `Promise` (MV3 only). It has no parameters.
   */
   ...
   *
   * @since Chrome 72.
   * @returns The `goBack` method provides its result via callback or returned as a `Promise` (MV3 only). It has no parameters.
   */
   ...
   *
   * @returns The `group` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   *
   * @returns The `ungroup` method provides its result via callback or returned as a `Promise` (MV3 only). It has no parameters.
   */
 ...
 * rearrange tab groups in the browser. To group and ungroup tabs, or to query what tabs are in groups, use the chrome.tabs API.
 *
...
declare namespace chrome.tabGroups {
  ...
  export interface MoveProperties {
    ...
    index: number;
    /**
     * Optional. The window to move the group to. Defaults to the window the group is currently in. Note that groups can
     * only be moved to and from windows with chrome.windows.WindowType type "normal".
     */
    windowId?: number;
  }
   ...
   * as well as for the group itself. This event is not fired when a group is moved between windows; instead, it will be
...
declare namespace chrome.tts {
  ...
  export interface TtsEvent {
    ...
    errorMessage?: string;
    /**
     * The type can be 'start' as soon as speech has started, 'word' when a word boundary is reached, 'sentence' when a
     * sentence boundary is reached, 'marker' when an SSML mark element is reached, 'end' when the end of the utterance
     * is reached, 'interrupted' when the utterance is stopped or interrupted before reaching the end, 'cancelled' when
     * it's removed from the queue before ever being synthesized, or 'error' when any other error occurs. When pausing
     * speech, a 'pause' event is fired if a particular utterance is paused in the middle, and 'resume' if an utterance
     * resumes speech. Note that pause and resume events may not fire if speech is paused in-between utterances.
     *
  ...
  export interface TtsVoice {
    ...
    eventTypes?: string[];
    /**
     * Optional. If true, the synthesis engine is a remote network resource. It may be higher latency and may incur
     * bandwidth costs.
     *
     * @since Chrome 33.
     */
  ...
  export interface SpeakOptions {
     ...
     * Optional.
     *
     * If true, enqueues this utterance if TTS is already in progress. If false (the default), interrupts any current
     ...
     * Optional.
     *
     * Speaking rate relative to the default rate for this voice. 1.0 is the default rate, normally around 180 to 220
     * words per minute. 2.0 is twice as fast, and 0.5 is half as fast. Values below 0.1 or above 10.0 are strictly
     * disallowed, but many voices will constrain the minimum and maximum rates further—for example a particular voice
     * may not actually speak faster than 3 times normal even if you specify a value larger than 3.0.
     */
    rate?: number;
    /**
     ...
     * Optional.
     *
     * Speaking pitch between 0 and 2 inclusive, with 0 being lowest and 2 being highest. 1.0 corresponds to a voice's
     * default pitch.
     */
    pitch?: number;
   ...
   *
   * @param utterance The text to speak, either plain text or a complete, well-formed SSML document. Speech engines that
   *   do not support SSML will strip away the tags and speak the text. The maximum length of the text is 32,768 characters.
   * @param callback Optional. Called right away, before speech finishes. Check chrome.runtime.lastError to make sure
 ...
 * registers using this API, it will receive events containing an utterance to be spoken and other parameters when any
 * extension or Chrome App uses the tts API to generate speech. Your extension can then use any available web technology
 * to synthesize and output the speech, and send events back to the calling function to report the status.
 *
...
declare namespace chrome.ttsEngine {
  export interface SpeakOptions {
     ...
     *
     * Speaking rate relative to the default rate for this voice. 1.0 is the default rate, normally around 180 to 220
     * words per minute. 2.0 is twice as fast, and 0.5 is half as fast. This value is guaranteed to be between 0.1 and
     * 10.0, inclusive. When a voice does not support this full range of rates, don't return an error. Instead, clip the
     * rate to the range the voice supports.
     */
    rate?: number;
    /**
     * Optional. Speaking pitch between 0 and 2 inclusive, with 0 being lowest and 2 being highest. 1.0 corresponds to
     * this voice's default pitch.
     */
    pitch?: number;
  }
...
declare namespace chrome.types {
  export interface ChromeSettingClearDetails {
     ...
     *
     * • incognito_session_only: setting for the incognito profile that can only be set during an incognito session and
     * is deleted when the incognito session ends (overrides regular and incognito_persistent preferences).
     */
    scope?: string;
  }
  ...
  export interface ChromeSettingSetDetails extends ChromeSettingClearDetails {
     ...
     * should not set a value of a different type.
     */
    value: any;
    /**
     * Optional.
...
declare namespace chrome.vpnProvider {
  export interface VpnSessionParameters {
    ...
    mtu?: string;
    /**
     * Exclude network traffic to the list of IP blocks in CIDR notation from the tunnel. This can be used to bypass
     * traffic to and from the VPN server. When many rules match a destination, the rule with the longest matching
     * prefix wins. Entries that correspond to the same CIDR block are treated as duplicates. Such duplicates in the
     * collated (exclusionList + inclusionList) list are eliminated and the exact duplicate entry that will be
     * eliminated is undefined.
     */
    exclusionList: string[];
    /**
     * Include network traffic to the list of IP blocks in CIDR notation to the tunnel. This parameter can be used to
     * set up a split tunnel. By default no traffic is directed to the tunnel. Adding the entry "0.0.0.0/0" to this list
     * gets all the user traffic redirected to the tunnel. When many rules match a destination, the rule with the
     * longest matching prefix wins. Entries that correspond to the same CIDR block are treated as duplicates. Such
     * duplicates in the collated (exclusionList + inclusionList) list are eliminated and the exact duplicate entry that
...
declare namespace chrome.wallpaper {
   ...
   *
   * @param callback Optional parameter thumbnail: The jpeg encoded wallpaper thumbnail. It is generated by resizing the
   *   wallpaper to 128x60.
   */
...
declare namespace chrome.webNavigation {
  export interface GetFrameDetails {
     ...
     *
     * @deprecated Since Chrome 49. Frames are now uniquely identified by their tab ID and frame ID; the process ID is
     *   no longer needed and therefore ignored.
     * @since Chrome 22.
     */
    processId?: number;
  ...
  export interface WebNavigationTransitionCallbackDetails extends WebNavigationFramedCallbackDetails {
     ...
     *
     * One of: "link", "typed", "auto_bookmark", "auto_subframe", "manual_subframe", "generated", "start_page",
     * "form_submit", "reload", "keyword", or "keyword_generated"
     */
  ...
  export interface WebNavigationEventFilter {
    /**
     * Conditions that the URL being navigated to must satisfy. The 'schemes' and 'ports' fields of UrlFilter are
   ...
   * @param callback Optional parameter details: Information about the requested frame, null if the specified frame ID
...
declare namespace chrome.webRequest {
  ...
  export interface BlockingResponse {
     ...
     * prevented from being sent/completed and is instead redirected to the given URL. Redirections to non-HTTP schemes
     * such as data: are allowed. Redirects initiated by a redirect action use the original request method for the
     * redirect, with one exception: If the redirect is initiated at the onHeadersReceived stage, then the redirect will
     * be issued using the GET method.
     */
    redirectUrl?: string;
    /**
     * Optional.
     ...
     * these response headers instead. Only return responseHeaders if you really want to modify the headers in order to
     * limit the number of conflicts (only one extension may modify responseHeaders for each request).
     */
  ...
  export interface WebRequestBody {
     ...
     * Optional.
     *
     * If the request method is POST and the body is a sequence of key-value pairs encoded in UTF8, encoded as either
     * multipart/form-data, or application/x-www-form-urlencoded, this dictionary is present and for each key contains
     * the list of all values for that key. If the data is of another media type, or if it is malformed, the dictionary
     * is not present. An example value of this dictionary is {'key': ['value1', 'value2']}.
     */
    formData?: { [key: string]: string[] };
    /**
     * Optional.
     *
     * If the request method is PUT or POST, and the body is not already parsed in formData, then the unparsed request
     * body elements are contained in this array.
     */
    raw?: UploadData[];
  }
  ...
  export interface ResourceRequest {
    ...
    timeStamp: number;
    /**
     * The origin where the request was initiated. This does not change through redirects. If this is an opaque origin,
     * the string 'null' will be used.
     *
  ...
  export interface WebResponseDetails extends ResourceRequest {
    /**
     * HTTP status line of the response or the 'HTTP/0.9 200 OK' string for HTTP/0.9 responses (i.e., responses that
     * lack a status line).
     */
    statusLine: string;
    /**
  ...
  export interface WebAuthenticationChallengeEvent
    ...
    > {
    addListener(
      callback: (details: WebAuthenticationChallengeDetails, callback?: (response: BlockingResponse) => void) => void,
      filter?: RequestFilter,
      opt_extraInfoSpec?: string[]
    ): void;
  }
...
declare namespace chrome.webstore {
  /**
   * @param url Optional. If you have more than one <link> tag on your page with the chrome-webstore-item relation, you
   *   can choose which item you'd like to install by passing in its URL here. If it is omitted, then the first (or
   *   only) link will be used. An exception will be thrown if the passed in URL does not exist on the page.
   * @param successCallback Optional. This function is invoked when inline installation successfully completes (after
   *   the dialog is shown and the user agrees to add the item to Chrome). You may wish to use this to hide the user
   *   interface element that prompted the user to install the app or extension.
   * @param failureCallback Optional. This function is invoked when inline installation does not successfully complete.
   *   Possible reasons for this include the user canceling the dialog, the linked item not being found in the store, or
   *   the install being initiated from a non-verified site.
   *
   *   Parameter error: The failure detail. You may wish to inspect or log this for debugging purposes, but you should not
   ...
   *   - - "otherError": An uncommon, unrecognized, or unexpected error. In some cases, the readable error string can provide
   *               more information.
 ...
 *
 * Permissions: The chrome.windows API can be used without declaring any permission. However, the "tabs" permission is
 * required in order to populate the url, title, and favIconUrl properties of Tab objects.
 *
 * @since Chrome 5.
 */
declare namespace chrome.windows {
  export interface Window {
    ...
    /**
     * Optional. The offset of the window from the top edge of the screen in pixels. Under some circumstances a Window
     * may not be assigned top property, for example when querying closed windows from the sessions API.
     */
    top?: number;
    /**
     * Optional. The height of the window, including the frame, in pixels. Under some circumstances a Window may not be
     * assigned height property, for example when querying closed windows from the sessions API.
     */
    height?: number;
    /**
     * Optional. The width of the window, including the frame, in pixels. Under some circumstances a Window may not be
    ...
    type?: windowTypeEnum;
    /**
     * Optional. The ID of the window. Window IDs are unique within a browser session. Under some circumstances a Window
     * may not be assigned an ID, for example when querying windows using the sessions API, in which case a session ID
     * may be present.
     */
    id?: number;
    /**
     * Optional. The offset of the window from the left edge of the screen in pixels. Under some circumstances a Window
     * may not be assigned left property, for example when querying closed windows from the sessions API.
     */
    left?: number;
    /**
  ...
  export interface QueryOptions {
     ...
     *
     * The Tab objects only contain the url, pendingUrl, title and favIconUrl properties if the extension's manifest
  ...
  export interface CreateData {
     ...
     * Optional.
     *
     * The number of pixels to position the new window from the top edge of the screen. If not specified, the new window
     * is offset naturally from the last focused window. This value is ignored for panels.
     */
    top?: number;
    /**
     * Optional.
    ...
    left?: number;
    /**
     * Optional. The initial state of the window. The 'minimized', 'maximized' and 'fullscreen' states cannot be
    ...
    state?: windowStateEnum;
    /**
     * If true, the newly-created window's 'window.opener' is set to the caller and is in the same [unit of related
     * browsing contexts](https://www.w3.org/TR/html51/browsers.html#unit-of-related-browsing-contexts) as the caller.
     *
     * @since Chrome 64.
     */
  ...
  export interface UpdateInfo {
    ...
    top?: number;
    /**
     * Optional. If true, causes the window to be displayed in a manner that draws the user's attention to the window,
     * without changing the focused window. The effect lasts until the user changes focus to the window. This option has
     * no effect if the window already has focus. Set to false to cancel a previous draw attention request.
     *
     * @since Chrome 14.
     */
    ...
    width?: number;
    /**
     * Optional. The new state of the window. The 'minimized', 'maximized' and 'fullscreen' states cannot be combined
  ...
  export interface WindowEventFilter {
    /**
     * Conditions that the window's type being created must satisfy. By default it will satisfy ['app', 'normal',
     * 'panel', 'popup'], with 'app' and 'panel' window types limited to the extension's own windows.
     */
    windowTypes: windowTypeEnum[];
  }
   ...
   * The state of this browser window.
   *
   * In some circumstances a window may not be assigned a state property; for example, when querying closed windows from
   * the sessions API.
   *
   * @since Chrome 44.
   */
   ...
   * The type of browser window this is.
   *
   * In some circumstances a window may not be assigned a type property; for example, when querying closed windows from
   * the sessions API.
   *
   * @since Chrome 44.
   */
   ...
   *
   * @returns The `create` method provides its result via callback or returned as a `Promise` (MV3 only). Contains
   *   details about the created window.
   */
   ...
   *
   * Note: On some Linux window managers, WINDOW_ID_NONE will always be sent immediately preceding a switch from one
...
declare namespace chrome.declarativeNetRequest {
  ...
  export interface RequestDetails {
    /**
     * The value 0 indicates that the request happens in the main frame; a positive value indicates the ID of a subframe
     * in which the request happens.
     *
     * If the document of a (sub-)frame is loaded (type is main_frame or sub_frame), frameId indicates the ID of this
     * frame, not the ID of the outer frame.
     *
  ...
  export interface RuleCondition {
     ...
     *
     * This is matched against a url where the host is encoded in the punycode format (in case of internationalized
     * domains) and any other non-ascii characters are url encoded in utf-8.
     */
    regexFilter?: string;
     ...
     *
     * Therefore urlFilter is composed of the following parts: (optional Left/Domain name anchor) + pattern + (optional
     * Right anchor).
     *
     ...
     * domains) and any other non-ascii characters are url encoded in utf-8.
     *
     * For example, when the request url is http://abc.рф?q=ф, the urlFilter will be matched against the url
   ...
   * Returns the number of static rules an extension can enable before the global static rule limit is reached.
   *
   * @returns The `getAvailableStaticRuleCount` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   * Returns the current set of dynamic rules for the extension.
   *
   * @returns The `getDynamicRules` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   * Returns the ids for the current set of enabled static rulesets.
   *
   * @returns The `getEnabledRulesets` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   *
   * This method is only available to extensions with the declarativeNetRequestFeedback permission or having the
   * activeTab permission granted for the tabId specified in filter.
   *
   ...
   * @param filter An object to filter the list of matched rules.
   *
   * @returns The `getMatchedRules` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   * Returns the current set of session scoped rules for the extension.
   *
   * @returns The `getSessionRules` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
   ...
   * @param regexOptions The regular expression to check.
   *
   * @returns The `isRegexSupported` method provides its result via callback or returned as a `Promise` (MV3 only).
   */
  export function isRegexSupported(regexOptions: RegexOptions): Promise<IsRegexSupportedResult>;

  /**
   * Configures if the action count for tabs should be displayed as the extension action's badge text and provides a way
   * for that action count to be incremented.
   */
   ...
   * for that action count to be incremented.
   *
   * @returns The `setExtensionActionOptions` method provides its result via callback or returned as a `Promise` (MV3
   *   only). It has no parameters.
   */
   ...
   *
   *   This can happen for multiple reasons, such as invalid rule format, duplicate rule ID, rule count limit exceeded,
   ...
   * MAX_NUMBER_OF_DYNAMIC_AND_SESSION_RULES is the maximum number of combined dynamic and session rules an extension can add.
   *
   * @returns The `updateDynamicRules` method provides its result via callback or returned as a `Promise` (MV3 only). It
   *   has no parameters.
   *
   *   In case of an error, runtime.lastError will be set and no change will be made to the rule set.
   *
   ...
   *
   * The rulesets with IDs listed in options.disableRulesetIds are first removed, and then the rulesets listed in
   * options.enableRulesetIds are added.
   *
   * Note that the set of enabled static rulesets is persisted across sessions but not across extension updates, i.e.
   * the rule_resources manifest key will determine the set of enabled static rulesets on each extension update.
   *
   ...
   * the rule_resources manifest key will determine the set of enabled static rulesets on each extension update.
   *
   * @returns The `updateEnabledRulesets` method provides its result via callback or returned as a `Promise` (MV3 only).
   ...
   * MAX_NUMBER_OF_DYNAMIC_AND_SESSION_RULES is the maximum number of combined dynamic and session rules an extension can add.
   *
   * @returns The `updateSessionRules` method provides its result via callback or returned as a `Promise` (MV3 only). It
   *   has no parameters.
   *
   *   In case of an error, runtime.lastError will be set and no change will be made to the rule set.
   *
   ...
   *
   * Only available for unpacked extensions with the declarativeNetRequestFeedback permission as this is intended to be
