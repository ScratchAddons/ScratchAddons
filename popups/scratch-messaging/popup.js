...
export default ({ addon, msg, safeMsg }) => ({
  ...
  computed: {
    uiMessages() {
      return {
        stMessagesMsg: this.msg("stMessages"),
        followsMsg: this.msg("follows"),
        studioInvitesMsg: this.msg("studio-invites"),
        forumMsg: this.msg("forum"),
        studioActivityMsg: this.msg("studio-activity"),
        remixesMsg: this.msg("remixes"),
        yourProfileMsg: this.msg("your-profile"),
        loadingMsg: this.msg("loading"),
        loggedOutMsg: this.msg("logged-out"),
        loadingCommentsMsg: this.msg("loading-comments"),
        reloadMsg: this.msg("reload"),
        dismissMsg: this.msg("dismiss"),
        noUnreadMsg: this.msg("no-unread"),
        showMoreMsg: this.msg("show-more"),
        markAsReadMsg: this.msg("mark-as-read"),
  ...
  created() {
    document.title = this.msg("popup-title");
    (async () => {
  ...
  methods: {
    ...
    dismissAlert(id) {
      const confirmation = confirm(this.msg("stMessagesConfirm"));
    ...
    async analyzeMessages(showAll = false) {
      ...
      const messagesToCheck = realMsgCount > 40 ? this.messages.length : showAll ? this.messages.length : realMsgCount;
    ...
    studioInviteHTML(invite) {
      ...
      return this.safeMsg("curate-invite", { actor, title });
    },
    ...
    forumHTML(forumTopic) {
      ...
      return this.safeMsg("forum-new-post", { title });
    },
    studioActivityHTML(studio) {
      ...
      return this.safeMsg("new-activity", { title });
    },
    remixHTML(remix) {
      const actor = `<a target="_blank"
            rel="noopener noreferrer"
    ...
    othersProfile(username) {
      return this.msg("others-profile", { username });
    },
    studioText(title) {
      return this.msg("studio", { title });
    },
