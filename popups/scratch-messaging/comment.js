...
export default {
  ...
  data() {
    return {
      ...
      messages: {
        openNewTabMsg: this.msg("open-new-tab"),
        deleteMsg: this.msg("delete"),
        deleteConfirmMsg: this.msg("delete-confirm"),
        replyMsg: this.msg("reply"),
        postingMsg: this.msg("posting"),
        postMsg: this.msg("post"),
        cancelMsg: this.msg("cancel"),
      },
    };
  },
  methods: {
      ...
      chrome.runtime.sendMessage(
        ...
        (res) => {
          ...
          if (res.error) {
            ...
            let errorMsg = this.msg(errorCode);
            if (res.muteStatus) {
              errorMsg = this.msg("comment-mute") + " ";
              errorMsg += this.msg("comment-cannot-post-for", {
    ...
    deleteComment() {
      ...
      this.thisComment.content = this.safeMsg("deleting");
      chrome.runtime.sendMessage(
        {
          scratchMessaging: {
            deleteComment: {
  ...
  computed: {
    ...
    replyBoxLeftMsg() {
      return this.msg("chars-left", { num: 500 - this.replyBoxValue.length });
    },
    username() {
    ...
    commentURL() {
      ...
      return `https://scratch.mit.edu/${urlPath}/${this.resourceId}/${commentPath}#comments-${this.commentId.substring(
        2
      )}`;
    },
  },
