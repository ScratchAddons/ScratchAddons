...
(async () => {
  ...
  chrome.storage.sync.get(["globalTheme"], function (/** True = light, false = dark. */ { globalTheme }) {
    if (globalTheme) {
  ...
  window.vue = new Vue({
    ...
    methods: {
      /**
       * @param {{ title: string; id: number; amt: number; users: string[]; extended: boolean }} projectObject
       * @param {number} i
       *
