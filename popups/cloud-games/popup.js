...
export default ({ addon, msg, safeMsg }) => ({
  ...
  data() {
    return {
      projects: [],
      loaded: false,
      messages: { noUsersMsg: this.msg("no-users") },
      projectsChecked: 0,
  ...
  computed: {
    ...
    loadingMsg() {
      return this.msg("loading", { done: this.projectsChecked, amount: this.projects.length || "?" });
    },
    errorMessage() {
      return this.error && this.msg(this.error);
    },
  },
  methods: {
    msg(...args) {
    ...
    settingsHTML() {
      ...
      link.textContent = this.msg("addon-settings");
      return this.safeMsg("change-studio", {
