...
const studioStrategy = async (studioId) => {
  let res;
  try {
    res = await traceableFetchExtension(`https://api.scratch.mit.edu/studios/${studioId}/projects/?limit=40`);
  } catch (e) {
...
const strategy = async (addon, displayedGames) =>
  Promise.all(
    displayedGames.map(async ({ id, type }) => {
      ...
      let res;
      try {
        res = await traceableFetchExtension(`https://api.scratch.mit.edu/projects/${id}`);
      } catch (e) {
